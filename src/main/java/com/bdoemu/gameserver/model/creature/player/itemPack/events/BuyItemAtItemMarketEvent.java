...
public class BuyItemAtItemMarketEvent extends AItemEvent {
	...
	public BuyItemAtItemMarketEvent(final Player player, final EItemStorageLocation srcStorageType,
			final long itemMarketObjectId, final long count, final int itemId, final int enchant, final int regionId) {
		super(player, player, player, EStringTable.eErrNoItemBuyFromItemMarket,
				EStringTable.eErrNoItemBuyFromItemMarket, regionId);
	...
	public void onEvent() {
		...
		World.getInstance().broadcastWorldPacket(new SMListItemHeaderAtItemMarket(
				Collections.singletonList(this.masterItemMarket), EPacketTaskType.Update));
		MailService.getInstance().sendMail(this.itemMarket.getAccountId(), 0L, "{3183609639|3119282326}",
				"{3183609639|990597375}", "{3183609639|199595621|643213191|ItemInfo:Name_" + this.itemMarket.getItemId()
						+ "|2207029135|" + this.count + "}");
	}
	...
	public boolean canAct() {
		...
		if (this.itemMarket == null) {
			this.player.sendPacket(new SMNotifyItemMarketInfo(ENotifyItemMarketInfoType.UPDATE, this.itemMarketObjectId,
					this.itemId, this.enchant));
			this.player.sendPacket(
					new SMNak(EStringTable.eErrNoAlreadySoldOutItemAtItemMarket, CMBuyItemAtItemMarket.class));
			return false;
		}
		if (this.itemMarket.getAccountId() == this.player.getAccountId()) {
			this.player.sendPacket(
					new SMNak(EStringTable.eErrNoItemExchangeCantExchangeWithMe, CMBuyItemAtItemMarket.class));
			return false;
		}
		this.revenue = this.itemMarket.getItemPrice() * this.count;
		this.masterItemMarket = ItemMarketService.getInstance().getMasterItemMarket(this.itemMarket.getItemId(),
				this.itemMarket.getEnchantLevel());
