...
public class ActionEffect extends ABuffEffect {
    @Override
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        final EActionEffectType type = EActionEffectType.values()[activeBuff.getTemplate().getParams()[0]];
        target.getActionStorage().setActionEffect(type);
    }
    ...
    public void endEffect(final ActiveBuff activeBuff) {
        final Creature target = activeBuff.getTarget();
        final EActionEffectType type = EActionEffectType.values()[activeBuff.getTemplate().getParams()[0]];
        target.getActionStorage().unsetActionEffect(type);
    }
}
