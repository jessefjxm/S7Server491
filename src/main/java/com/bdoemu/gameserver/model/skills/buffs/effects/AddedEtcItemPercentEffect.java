...
public class AddedEtcItemPercentEffect extends ABuffEffect {
    @Override
    public Collection<ActiveBuff> initEffect(final Creature owner, final Collection<? extends Creature> targets, final SkillT skillT, final BuffTemplate buffTemplate) {
        ...
        buffs.addAll(targets.stream().map(target -> new ActiveBuff(skillT, buffTemplate, owner, target, element)).collect(Collectors.toList()));
        return buffs;
    }
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        target.getGameStats().getDoubleDropItemLuck().addElement(element);
    }
    ...
    public void endEffect(final ActiveBuff activeBuff) {
        final BuffElement element = activeBuff.getElement();
        final Creature target = activeBuff.getTarget();
        target.getGameStats().getDoubleDropItemLuck().removeElement(element);
    }
}
