...
public class AquireRateHighKnowledgeEffect extends ABuffEffect {
    @Override
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        target.getGameStats().getAquireHighKnowledgeRateStat().addElement(element);
    }
    ...
    public void endEffect(final ActiveBuff activeBuff) {
        final BuffElement element = activeBuff.getElement();
        final Creature target = activeBuff.getTarget();
        target.getGameStats().getAquireHighKnowledgeRateStat().removeElement(element);
    }
}
