...
public class AttackSpeedVariationRateEffect extends ABuffEffect {
    @Override
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        target.getGameStats().getAttackSpeedRate().addRateElement(activeBuff.getElement());
        if (target.isPlayer()) {
    ...
    public void endEffect(final ActiveBuff activeBuff) {
        final Creature target = activeBuff.getTarget();
        target.getGameStats().getAttackSpeedRate().removeRateElement(activeBuff.getElement());
        if (target.isPlayer()) {
