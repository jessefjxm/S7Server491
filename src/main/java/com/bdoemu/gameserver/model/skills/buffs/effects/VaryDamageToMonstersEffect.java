...
public class VaryDamageToMonstersEffect extends ABuffEffect {
    @Override
    public Collection<ActiveBuff> initEffect(final Creature owner, final Collection<? extends Creature> targets, final SkillT skillT, final BuffTemplate buffTemplate) {
        final List<ActiveBuff> buffs = new ArrayList<ActiveBuff>();
        final Integer[] params = buffTemplate.getParams();
        final int monsterAddDamage = params[0];
        final BuffElement element = new BuffElement(monsterAddDamage);
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        target.getGameStats().getEtcAttackDamageStat().addElement(activeBuff.getElement());
    }

    @Override
    public void endEffect(final ActiveBuff activeBuff) {
        final Creature target = activeBuff.getTarget();
        target.getGameStats().getEtcAttackDamageStat().removeElement(activeBuff.getElement());
    }
}
