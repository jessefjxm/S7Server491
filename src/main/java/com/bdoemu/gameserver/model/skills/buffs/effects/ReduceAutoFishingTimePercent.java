...
public class ReduceAutoFishingTimePercent extends ABuffEffect {
    @Override
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        target.getGameStats().getAutoFishingTimeReduce().addElement(element);
    }
    ...
    public void endEffect(final ActiveBuff activeBuff) {
        final Creature target = activeBuff.getTarget();
        final BuffElement element = activeBuff.getElement();
        target.getGameStats().getAutoFishingTimeReduce().removeElement(element);
    }
}
