...
public class ActionResistanceEffect extends ABuffEffect {
    @Override
    public Collection<ActiveBuff> initEffect(final Creature owner, final Collection<? extends Creature> targets, final SkillT skillT, final BuffTemplate buffTemplate) {
        final List<ActiveBuff> buffs = new ArrayList<>();
        final Integer[] params = buffTemplate.getParams();
        final int resistPercents = params[1];
        final BuffElement element = new BuffElement(resistPercents, true);
    ...
    public void applyEffect(final ActiveBuff activeBuff) {
        ...
        if (target.isPlayer()) {
            target.sendPacket(new SMSetCharacterPrivatePoints(target));
        }
    }
