...
@IAIName("novicelandspiderqueen_boss")
public class Ai_novicelandspiderqueen_boss extends CreatureAI {
	public Ai_novicelandspiderqueen_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Summon_Attack(double blendTime) {
		...
		doAction(3697722837L /*BATTLE_SUMMON1*/, blendTime, onDoActionEnd -> changeState(state -> Summon_Logic(blendTime)));
	}

	protected void Summon_Attack2(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x984B765L /*Summon_Attack2*/);
		if (isTargetLost()) {
			if (changeState(state -> Logic(blendTime)))
				return;
		}
		doAction(3814969495L /*BATTLE_SUMMON2*/, blendTime, onDoActionEnd -> changeState(state -> Summon_Logic(blendTime)));
	}
	...
	protected void Summon_Logic(double blendTime) {
		...
		getObjects(EAIFindTargetType.Monster, object -> (getTargetCharacterKey(object) == 20573 || getTargetCharacterKey(object) == 20574 || getTargetCharacterKey(object) == 20575 || getTargetCharacterKey(object) == 20576)).forEach(consumer -> consumer.getAi()._helpme(getActor(), null));
		changeState(state -> Battle_Wait(blendTime));
	}
