...
@IAIName("m0412_camamanshagoblin")
public class Ai_m0412_camamanshagoblin extends CreatureAI {
	public Ai_m0412_camamanshagoblin(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Run(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) <= 100) {
			if (changeState(state -> Battle_Chargeattack1(0.5)))
				return;
		}
	...
	protected void Move_Back(double blendTime) {
		...
		doAction(1516377807L /*MOVE_BACK*/, blendTime, onDoActionEnd -> scheduleState(state -> Move_Back_Ing(blendTime), 100));
	}
	...
	protected void Move_Back_Ing(double blendTime) {
		...
		doAction(407722945L /*MOVE_BACK_ING*/, blendTime, onDoActionEnd -> escape(1000, () -> {
			return false;
		}, onExit -> scheduleState(state -> Move_Back_End(blendTime), 500)));
	}

	protected void Move_Back_End(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xBABDB2B6L /*Move_Back_End*/);
		if (isTargetLost()) {
			if (changeState(state -> LostTarget(blendTime)))
				return;
		}
		doAction(2459669163L /*MOVE_BACK_END*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
	...
	protected void Battle_Attack1(double blendTime) {
		...
		doAction(31162842L /*BATTLE_ATTACK1*/, blendTime, onDoActionEnd -> {
			if(Rnd.getChance(20)) {
				if (changeState(state -> Move_Back(0.4)))
					return;
			}
			if (changeState(state -> Battle_Wait(0.4)))
				return;
	...
	protected void Battle_Attack3(double blendTime) {
		...
		doAction(931985982L /*BATTLE_ATTACK3*/, blendTime, onDoActionEnd -> {
			if(Rnd.getChance(20)) {
				if (changeState(state -> Move_Back(0.4)))
					return;
			}
	...
	protected void Battle_Chargeattack1(double blendTime) {
		...
		doAction(3904990386L /*BATTLE_CHARGEATTACK1*/, blendTime, onDoActionEnd -> {
			if(Rnd.getChance(20)) {
				if (changeState(state -> Move_Back(0.4)))
					return;
			}
