...
@IAIName("desertdragon_boss_event")
public class Ai_desertdragon_boss_event extends CreatureAI {
	public Ai_desertdragon_boss_event(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void CheckSpawn_Logic(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, true, object -> getDistanceToTarget(object) < 4000 && getTargetCharacterKey(object) == 23032)) {
	...
	protected void Notifier_Logic(double blendTime) {
		...
		worldNotify(EChatNoticeType.Nuberu, "GAME", "LUA_EVENT_DESERTDRAGON_SQAWN");
