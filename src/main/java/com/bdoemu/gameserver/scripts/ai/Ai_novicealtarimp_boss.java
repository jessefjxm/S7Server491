...
@IAIName("novicealtarimp_boss")
public class Ai_novicealtarimp_boss extends CreatureAI {
	public Ai_novicealtarimp_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		doAction(3630482305L /*AI_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Summon_Start(blendTime), 1000));
	}
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) < 600 && getVariable(0x3F487035L /*_Hp*/) <= 50 && getVariable(0x3826DD12L /*_CrazySwingAttCount*/) >= 10 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if(Rnd.getChance(30)) {
	...
	protected void Attack_CrazySwing_Ing(double blendTime) {
		...
		doAction(3179342358L /*ATTACK_CRAZYSWING_LOOP*/, blendTime, onDoActionEnd -> changeState(state -> Attack_CrazySwing_Logic(blendTime)));
	}
	...
	public EAiHandlerResult HandleTakeDamage(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getVariable(0x3826DD12L /*_CrazySwingAttCount*/) < 25) {
