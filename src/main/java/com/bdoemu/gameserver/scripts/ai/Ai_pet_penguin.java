...
@IAIName("pet_penguin")
public class Ai_pet_penguin extends CreatureAI {
	public Ai_pet_penguin(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait_Summon_Logic(double blendTime) {
		...
		if(Rnd.getChance(15)) {
			if (changeState(state -> Random_Move2_Str(0.3)))
				return;
		}
		if(Rnd.getChance(15)) {
	...
	protected void Random_Move2_Str(double blendTime) {
		...
		doAction(1000628648L /*SLIDING_STR*/, blendTime, onDoActionEnd -> scheduleState(state -> Random_Move2_Ing(blendTime), 1000));
	}
	...
	protected void Random_Move2_Ing(double blendTime) {
		...
		doAction(2948328545L /*SLIDING_ING*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 50, 300, false, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> Random_Move2_End(blendTime), 1000)));
	}
	...
	protected void Random_Move2_End(double blendTime) {
		...
		doAction(167981748L /*SLIDING_END*/, blendTime, onDoActionEnd -> scheduleState(state -> Wait_Summon_Logic(blendTime), 1000));
	}
	...
	protected void ChaseOwner_MoveLv3(double blendTime) {
		...
		doAction(169952953L /*SLIDING_FAST_STR*/, blendTime, onDoActionEnd -> scheduleState(state -> ChaseOwner_MoveLv3_Ing(blendTime), 1000));
	}
	...
	protected void ChaseOwner_MoveLv3_Ing(double blendTime) {
		...
		doAction(2846054261L /*SLIDING_FAST_ING*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPosition, 0, 0, 0, 40, 100, false, ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0 && getVariable(0xFDC61BCCL /*_FailFindPathCount*/) >= 3) {
				if (changeState(state -> FailFindPath_Summon(0.3)))
					return true;
			...
			return false;
		}, onExit -> scheduleState(state -> ChaseOwner_MoveLv3_Ing(blendTime), 1500)));
	}
	...
	protected void ChaseOwner_MoveLv3_End(double blendTime) {
		...
		doAction(4215928511L /*SLIDING_FAST_END*/, blendTime, onDoActionEnd -> {
			if (getVariable(0xCBEEF8C7L /*_ownerDistance*/) < 280) {
				if (changeState(state -> ChaseOwner_MoveLv1(blendTime)))
					return;
			}
