...
@IAIName("novicecatfishmanfat_boss")
public class Ai_novicecatfishmanfat_boss extends CreatureAI {
	public Ai_novicecatfishmanfat_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		doAction(2155892455L /*WAIT_START*/, blendTime, onDoActionEnd -> scheduleState(state -> Start_Action(blendTime), 1000));
	}
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0xD3505174L /*_Summon_Count*/) == 5 && getVariable(0x3F487035L /*_HP*/) > 80 && getPartyMembersCount()< 3 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if (changeState(state -> Summon(0.3)))
				return;
		}
		if (getVariable(0x3F487035L /*_HP*/) < 50 && getVariable(0xD3505174L /*_Summon_Count*/) == 2 && getPartyMembersCount()< 3 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if (changeState(state -> Summon2(0.3)))
				return;
		}
	...
	protected void Charge_Attack(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x849807DDL /*Charge_Attack*/);
		if (isTargetLost()) {
			if (changeState(state -> Logic(blendTime)))
				return;
		}
