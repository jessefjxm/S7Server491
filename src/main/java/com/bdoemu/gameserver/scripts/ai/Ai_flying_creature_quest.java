...
@IAIName("flying_creature_quest")
public class Ai_flying_creature_quest extends CreatureAI {
	public Ai_flying_creature_quest(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void FallIng_Logic(double blendTime) {
		...
		scheduleState(state -> Flying_Random(blendTime), 300);
	}
	...
	protected void Flying_Random(double blendTime) {
		...
		doAction(1514014862L /*FLYING_CURVE_MOVE*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 500, 1800, 2500, false, ENaviType.ground, () -> {
			return false;
	...
	protected void Flying_Random_RetryCurveTurn(double blendTime) {
		...
		doAction(3426992848L /*FLYING_CURVE_TURN*/, blendTime, onDoActionEnd -> turn(EAITurnMethod.ToPathCurve, 0, () -> {
			return false;
		}, onExit -> scheduleState(state -> Flying_Random_RetryCurve(blendTime), 500)));
	}

	protected void Flying_Random_RetryCurve(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.walk);
		setState(0x17F3C653L /*Flying_Random_RetryCurve*/);
		if (isTargetLost()) {
			if (changeState(state -> FlyingWait(blendTime)))
				return;
		}
		...
		doAction(1514014862L /*FLYING_CURVE_MOVE*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.RetryCurve, 0, 0, 0, 0, 10, false, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> FlyingWait(blendTime), 500)));
	}
	...
	protected void FailFindPath_Summon(double blendTime) {
		...
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Flying_Random(blendTime), 500));
	}
