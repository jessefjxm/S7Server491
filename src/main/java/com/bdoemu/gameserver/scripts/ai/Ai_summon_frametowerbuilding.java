...
@IAIName("summon_frametowerbuilding")
public class Ai_summon_frametowerbuilding extends CreatureAI {
	public Ai_summon_frametowerbuilding(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Working_Str(double blendTime) {
		...
		useSkill(41062, 1, false, EAIFindTargetType.Self, object -> true);
	...
	protected void Wait_Logic(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.AllyVehicle, EAIFindType.normal, true, object -> getTargetCharacterKey(object) == 9082 && getDistanceToTarget(object) < 100)) {
	...
	protected void Die(double blendTime) {
		...
		getObjects(EAIFindTargetType.Character, object -> getDistanceToTarget(object) < 100 && getTargetCharacterKey(object) == 9082).forEach(consumer -> consumer.getAi()._DestroyedBase(getActor(), null));
