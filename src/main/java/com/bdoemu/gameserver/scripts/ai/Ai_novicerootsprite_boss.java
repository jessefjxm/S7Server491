...
@IAIName("novicerootsprite_boss")
public class Ai_novicerootsprite_boss extends CreatureAI {
	public Ai_novicerootsprite_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0x3F487035L /*_HP*/) < 50 && target != null && getDistanceToTarget(target) < 800 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if(Rnd.getChance(15)) {
				if (changeState(state -> Attack_Bomb(0.3)))
					return;
			}
		}
	...
	protected void Attack_Bomb(double blendTime) {
		...
		doAction(2886130280L /*BATTLE_ATTACK_BOMB*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
	...
	protected void Attack_Turn(double blendTime) {
		...
		doAction(3304437007L /*BATTLE_ATTACK_TURN*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
	...
	protected void Attack_Dash(double blendTime) {
		...
		doAction(2067132864L /*BATTLE_DASH_RUN_STR*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
