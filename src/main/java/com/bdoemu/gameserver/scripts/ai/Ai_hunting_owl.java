...
@IAIName("hunting_owl")
public class Ai_hunting_owl extends CreatureAI {
	public Ai_hunting_owl(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Fall_Start(double blendTime) {
		...
		doAction(3495010968L /*FALLING_STR*/, blendTime, onDoActionEnd -> scheduleState(state -> Fall(blendTime), 3000));
	}
	...
	protected void Fall_End(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xB833A44L /*Fall_End*/);
		doAction(3399936975L /*FALLING_END*/, blendTime, onDoActionEnd -> scheduleState(state -> Die(blendTime), 3000));
	}
