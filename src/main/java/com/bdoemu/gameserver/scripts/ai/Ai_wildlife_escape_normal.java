...
@IAIName("wildlife_escape_normal")
public class Ai_wildlife_escape_normal extends CreatureAI {
	public Ai_wildlife_escape_normal(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Escape(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) < getVariable(0xBA482B0EL /*AI_ES_RunDistance*/)) {
			if (changeState(state -> Escape_Run(0.3)))
				return;
		}
		if (target != null && getDistanceToTarget(target) < getVariable(0xBA482B0EL /*AI_ES_RunDistance*/) && target != null && (getTargetCharacterKey(target) == getVariable(0xED8CCD5AL /*AI_Escape_CharacterKey1*/) || getTargetCharacterKey(target) == getVariable(0x55BCA4AFL /*AI_Escape_CharacterKey2*/) || getTargetCharacterKey(target) == getVariable(0x9F57720DL /*AI_Escape_CharacterKey3*/) || getTargetCharacterKey(target) == getVariable(0xBFEB4186L /*AI_Escape_CharacterKey4*/) || getTargetCharacterKey(target) == getVariable(0xADEC46DDL /*AI_Escape_CharacterKey5*/)) && target != null && isCreatureVisible(target, false)) {
			if (changeState(state -> Escape_Run(0.3)))
				return;
		}
		if (getDistanceToSpawn() > 6000) {
	...
	public EAiHandlerResult HandleTargetInMyArea(Creature sender, Integer[] eventData) {
		...
		if ((getState() == 0x866C7489L /*Wait*/ || getState() == 0x8377635AL /*Move_Random*/) && target != null && isCreatureVisible(target, false)) {
	...
	public EAiHandlerResult HandleTakeDamage(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getVariable(0x6B36CE54L /*_DamageCount*/) <= 6) {
			setVariable(0x6B36CE54L /*_DamageCount*/, getVariable(0x6B36CE54L /*_DamageCount*/) + 1);
		}
		if (getVariable(0x6B36CE54L /*_DamageCount*/) >= 5) {
