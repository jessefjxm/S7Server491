...
@IAIName("kamawoodawaken_boss")
public class Ai_kamawoodawaken_boss extends CreatureAI {
	public Ai_kamawoodawaken_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Before_Die(double blendTime) {
		...
		doAction(1174134199L /*BEFORE_DIE*/, blendTime, onDoActionEnd -> {
			getObjects(EAIFindTargetType.Sibling, object -> getTargetCharacterKey(object) == 23767).forEach(consumer -> consumer.getAi().Die(getActor(), null));
	...
	protected void Disappear(double blendTime) {
		...
		doAction(1903499173L /*DISAPPEAR*/, blendTime, onDoActionEnd -> {
			getObjects(EAIFindTargetType.Sibling, object -> getTargetCharacterKey(object) == 23767).forEach(consumer -> consumer.getAi().Revival(getActor(), null));
	...
	protected void Order(double blendTime) {
		...
		getObjects(EAIFindTargetType.Sibling, object -> getTargetCharacterKey(object) == 23767).forEach(consumer -> consumer.getAi().Order(getActor(), null));
