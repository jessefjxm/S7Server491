...
@IAIName("imp_boss")
public class Ai_imp_boss extends CreatureAI {
	public Ai_imp_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		doAction(3910697700L /*START_ACTION*/, blendTime, onDoActionEnd -> scheduleState(state -> Wait(blendTime), 100));
	}
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0xE8587A5L /*_Special_Stun_EndTime*/) >= 20000) {
			if (findTarget(EAIFindTargetType.Enemy, EAIFindType.normal, false, object -> getDistanceToTarget(object, false) >= 0 && getDistanceToTarget(object, false) <= 500 && getTargetHp(object) > 0 && getTargetAction(object) == 0x580AEEBDL /*MiniGame0*/)) {
	...
	protected void Attack_Combination(double blendTime) {
		...
		setVariable(0xBEE22793L /*_CombinationCount*/, 10);
	...
	protected void Attack_Roar_Roar(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x1A0A682L /*Attack_Roar_Roar*/);
		setVariable(0x7398A4FL /*_IsRoar*/, 1);
		doAction(93247687L /*ATTACK_ROAR*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
