...
@IAIName("party_leader_ru_treedummer")
public class Ai_party_leader_ru_treedummer extends CreatureAI {
	public Ai_party_leader_ru_treedummer(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Move_Teleport_Logic(double blendTime) {
		...
		if (getVariable(0xAA320A99L /*_DiceCount*/) <= 33) {
			if (changeState(state -> Move_SummonPoint1(blendTime)))
				return;
		}
		if (getVariable(0xAA320A99L /*_DiceCount*/) > 33 && getVariable(0xAA320A99L /*_DiceCount*/) <= 66) {
			if (changeState(state -> Move_SummonPoint2(blendTime)))
				return;
		}
		if (getVariable(0xAA320A99L /*_DiceCount*/) > 66) {
	...
	protected void Move_SummonPoint1(double blendTime) {
		...
		doTeleportToWaypoint("ru_treedummer_boss_001", "monster_patrol", 0, 0, 1, 1);
	...
	protected void Move_SummonPoint2(double blendTime) {
		...
		doTeleportToWaypoint("ru_treedummer_boss_002", "monster_patrol", 0, 0, 1, 1);
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Summon_Wait(blendTime), 1000));
	}
	...
	protected void Move_SummonPoint3(double blendTime) {
		...
		doTeleportToWaypoint("ru_treedummer_boss_003", "monster_patrol", 0, 0, 1, 1);
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Summon_Wait(blendTime), 1000));
	}
