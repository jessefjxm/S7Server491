...
@IAIName("carriage")
public class Ai_carriage extends CreatureAI {
	public Ai_carriage(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Riding_Wait(double blendTime) {
		...
		doAction(2748324682L /*CARRIAGE_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> TerminateState(blendTime), 100));
	}
	...
	public EAiHandlerResult HandleWhistle(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> OwnerTeleport(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
