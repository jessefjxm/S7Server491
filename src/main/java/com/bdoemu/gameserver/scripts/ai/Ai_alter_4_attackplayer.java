...
@IAIName("alter_4_attackplayer")
public class Ai_alter_4_attackplayer extends CreatureAI {
	public Ai_alter_4_attackplayer(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Move_Defence_Chaser(double blendTime) {
		...
		doAction(825954671L /*MOVE_DEFENCE_CHASER*/, blendTime, onDoActionEnd -> chase(getVariable(0xD2BFE684L /*TargetCycle*/), () -> {
			...
			return false;
		}, onExit -> scheduleState(state -> Move_Defence_Chaser(blendTime), 100)));
	}
	...
	protected void Target_Change(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Player, EAIFindType.nearest, false, object -> getDistanceToTarget(object) < 2000)) {
			if (changeState(state -> HighSpeed_Chaser(blendTime)))
				return;
		}
		if (checkParentInstanceTeamNo()) {
			if (findTarget(EAIFindTargetType.Monster, EAIFindType.nearest, false, object -> getDistanceToTarget(object) < 10000 && getTargetCharacterKey(object) == 27137)) {
				if (changeState(state -> Return_Shrine(blendTime)))
					return;
			}
		}
	...
	protected void Move_Return(double blendTime) {
		...
		if (checkParentInstanceTeamNo()) {
			if (findTarget(EAIFindTargetType.Monster, EAIFindType.nearest, true, object -> getDistanceToTarget(object) < 10000 && getTargetCharacterKey(object) == 27137)) {
				if (changeState(state -> Return_Shrine(blendTime)))
					return;
			}
		}
	...
	protected void Return_Shrine(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) > 200 && target != null && getDistanceToTarget(target) < 1500) {
		...
		doAction(3208508703L /*CHASER_RUN*/, blendTime, onDoActionEnd -> chase(10, () -> {
			...
			return false;
		}, onExit -> scheduleState(state -> Return_Shrine(blendTime), 5000)));
	}
