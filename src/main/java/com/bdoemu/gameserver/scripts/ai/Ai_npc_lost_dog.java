...
@IAIName("npc_lost_dog")
public class Ai_npc_lost_dog extends CreatureAI {
	public Ai_npc_lost_dog(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Move_Destination(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, false, object -> getDistanceToTarget(object) < 500)) {
			if (changeState(state -> FearStop(0.2)))
				return;
		}
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.WaypointVariable, getVariableArray(0xACD660AFL /*WaypointValue*/), ENaviType.ground, () -> {
			...
			return false;
		}, onExit -> scheduleState(state -> Stop1(blendTime), 5000)));
	}

	protected void FearStop(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x671EA619L /*FearStop*/);
		if (isTargetLost()) {
			if (changeState(state -> Move_Destination(blendTime)))
				return;
		}
		doAction(927442757L /*WAIT1*/, blendTime, onDoActionEnd -> changeState(state -> FearStop(blendTime)));
	}
	...
	protected void Stop1(double blendTime) {
		...
		doAction(3401480022L /*WAIT2*/, blendTime, onDoActionEnd -> changeState(state -> Stop1(blendTime)));
	}
