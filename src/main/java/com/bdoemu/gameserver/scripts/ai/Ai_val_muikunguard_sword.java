...
@IAIName("val_muikunguard_sword")
public class Ai_val_muikunguard_sword extends CreatureAI {
	public Ai_val_muikunguard_sword(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait(double blendTime) {
		...
		setVariable(0xFA9DA674L /*_IsBattleMode*/, 0);
		if (findTarget(EAIFindTargetType.Player, EAIFindType.normal, false, object -> getDistanceToTarget(object) < 1200 && getTargetHp(object) > 0 && getTendency(object) >= 0 && isCreatureVisible(object, false))) {
	...
	protected void Move_Random(double blendTime) {
		...
		doAction(633942026L /*MOVE_WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 100, 300, false, ENaviType.ground, () -> {
			return false;
	...
	protected void Move_Chaser(double blendTime) {
		...
		if (target != null && (getDistanceToTarget(target, false) >= 0 && getDistanceToTarget(target, false) <= 200) && target != null && getTargetHp(target) > 0) {
			if(Rnd.getChance(50)) {
				if (changeState(state -> Battle_Attack(0.3)))
					return;
			}
	...
	protected void Move_ParentPath(double blendTime) {
		...
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPath, 0, 0, 0, 0, 350, false, ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
				if (changeState(state -> FailFindPath_Logic(0.3)))
					return true;
