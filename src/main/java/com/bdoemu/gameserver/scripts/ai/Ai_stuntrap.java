...
@IAIName("stuntrap")
public class Ai_stuntrap extends CreatureAI {
	public Ai_stuntrap(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> AttackStun(blendTime), 1000));
	}
	...
	protected void AttackStun(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x62944217L /*AttackStun*/);
		doAction(978762002L /*ATTACK_STUN*/, blendTime, onDoActionEnd -> changeState(state -> UnSummon(blendTime)));
	}
