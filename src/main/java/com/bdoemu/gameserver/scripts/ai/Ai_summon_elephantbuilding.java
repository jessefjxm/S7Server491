...
@IAIName("summon_elephantbuilding")
public class Ai_summon_elephantbuilding extends CreatureAI {
	public Ai_summon_elephantbuilding(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Working_Str(double blendTime) {
		...
		if (getVariable(0xE26EA7A0L /*_EndTime*/) >= 720000) {
			if (changeState(state -> Working_End(blendTime)))
				return;
		}
		doAction(3058152320L /*WORKING_STR*/, blendTime, onDoActionEnd -> scheduleState(state -> Working_Str(blendTime), 7200));
	}
