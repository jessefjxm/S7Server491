...
@IAIName("summon_catapultbuilding")
public class Ai_summon_catapultbuilding extends CreatureAI {
	public Ai_summon_catapultbuilding(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Working_Str(double blendTime) {
		...
		useSkill(41061, 1, false, EAIFindTargetType.Self, object -> true);
	...
	protected void Wait_Logic(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.AllyVehicle, EAIFindType.normal, true, object -> getTargetCharacterKey(object) == 9083 && getDistanceToTarget(object) < 100)) {
	...
	protected void Die(double blendTime) {
		...
		getObjects(EAIFindTargetType.Character, object -> getDistanceToTarget(object) < 100 && getTargetCharacterKey(object) == 9083).forEach(consumer -> consumer.getAi()._DestroyedBase(getActor(), null));
