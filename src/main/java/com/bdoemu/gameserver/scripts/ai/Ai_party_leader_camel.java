...
@IAIName("party_leader_camel")
public class Ai_party_leader_camel extends CreatureAI {
	public Ai_party_leader_camel(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void FailFindPath_Logic(double blendTime) {
		...
		if (getVariable(0xFDC61BCCL /*_FailFindPathCount*/) > 3 && getVariable(0xFA9DA674L /*_isBattleMode*/) == 0) {
			if (changeState(state -> FailFindPath_ReturnToParent(0.3)))
				return;
		}
		if (getVariable(0xFDC61BCCL /*_FailFindPathCount*/) > 3 && getVariable(0xFA9DA674L /*_isBattleMode*/) == 1) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) <= 1500 && target != null && getTargetHp(target) > 0 && target != null && isCreatureVisible(target, false)) {
			if (changeState(state -> Battle_Cheer(0.3)))
				return;
		}
		if (target != null && getDistanceToTarget(target) > 1500 && target != null && getTargetHp(target) > 0 && target != null && isCreatureVisible(target, false)) {
	...
	protected void Battle_Cheer(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x250172E0L /*Battle_Cheer*/);
		if (isTargetLost()) {
		...
		doAction(1214751792L /*BATTLE_CHEERUP*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
	...
	protected void Move_ParentPath(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Player, EAIFindType.normal, false, object -> getDistanceToTarget(object) < 3500 && getTargetHp(object) > 0 && getTendency(object) < 0 && isCreatureVisible(object, false))) {
			if (changeState(state -> Battle_Wait(0.3)))
				return;
		}
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPosition, 0, 0, 0, 0, 1, false, ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
				if (changeState(state -> FailFindPath_Logic(0.3)))
					return true;
	...
	public EAiHandlerResult HandleTargetInMyArea(Creature sender, Integer[] eventData) {
		...
		if ((getState() == 0x866C7489L /*Wait*/ || getState() == 0x8377635AL /*Move_Random*/ || getState() == 0x67695F37L /*Lost_Target*/ || getState() == 0x838FBB02L /*Move_ParentPath*/) && target != null && getTargetHp(target) > 0 && target != null && getTendency(target) < 0 && target != null && isCreatureVisible(target, false)) {
	...
	public EAiHandlerResult HandleTakeDamage(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getState() != 0x71F28994L /*Battle_Wait*/ && getState() != 0x250172E0L /*Battle_Cheer*/ && getState() != 0x39B3FBC2L /*Move_Chaser*/) {
			if (changeState(state -> Battle_Wait(0.3)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
	...
	public EAiHandlerResult HandleBrotherCall(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Lost_Target(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
