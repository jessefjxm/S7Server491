...
@IAIName("alter_ancientbird_boss")
public class Ai_alter_ancientbird_boss extends CreatureAI {
	public Ai_alter_ancientbird_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Teleport_Logic(double blendTime) {
		...
		doTeleport(EAIMoveDestType.Absolute, -374761, -293014, 0, 0);
		changeState(state -> Ready(blendTime));
	}
	...
	protected void Ready(double blendTime) {
		...
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Ready(blendTime), 1000));
	}
	...
	public EAiHandlerResult Attack_Start(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getState() == 0xA8A18440L /*Ready*/) {
			if (changeState(state -> Rotate_Logic(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
	}
