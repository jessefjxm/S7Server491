...
@IAIName("asiantoad_boss")
public class Ai_asiantoad_boss extends CreatureAI {
	public Ai_asiantoad_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0x7B4DEA05L /*_CrazyCount*/) == 1 && target != null && getAngleToTarget(target) <= -25 && target != null && getDistanceToTarget(target) < 300) {
			if(Rnd.getChance(30)) {
				if (changeState(state -> Battle_Attack_Jump(0.2)))
					return;
			}
		}
		...
		if (getVariable(0x3F487035L /*_HP*/) > 70 && getVariable(0x7B4DEA05L /*_CrazyCount*/) == 0 && target != null && getDistanceToTarget(target) < 500 && getVariable(0x69F71296L /*_Bomber_Sow*/) >= 12) {
			if(Rnd.getChance(30)) {
		...
		if (target != null && getDistanceToTarget(target) > 500) {
			if (changeState(state -> Attack_Jump_Chase(0.2)))
				return;
		}
	...
	protected void Attack_Jump_Chase(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xFB8AE583L /*Attack_Jump_Chase*/);
		if (isTargetLost()) {
			if (changeState(state -> Logic(blendTime)))
				return;
		}
		...
		doAction(630635331L /*ATTACK_JUMP_CHASE_READY*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Jump_Chase(blendTime)));
	}

	protected void Attack_Jump_Chase_Logic(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.none);
		setState(0x86ADC76AL /*Attack_Jump_Chase_Logic*/);
		...
		changeState(state -> Attack_Jump_Chase(blendTime));
	}
	...
	protected void Battle_Attack_Jump(double blendTime) {
		...
		doAction(116403464L /*ATTACK_JUMP_TURN*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
	...
	protected void AttackRange_Normal(double blendTime) {
		...
		doAction(3769038258L /*ATTACKRANGE_NORMAL*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
	...
	protected void AttackRange_Multi_A(double blendTime) {
		...
		doAction(3867160443L /*ATTACKRANGE_MULTI_A*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
	...
	protected void AttackSkill_Bomber_Sow(double blendTime) {
		...
		setVariable(0x69F71296L /*_Bomber_Sow*/, 0);
		doAction(243208646L /*ATTACKSKILL_BOMBER_SOW_READY*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
