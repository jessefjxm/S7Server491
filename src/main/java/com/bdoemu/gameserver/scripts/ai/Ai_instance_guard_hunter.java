...
@IAIName("instance_guard_hunter")
public class Ai_instance_guard_hunter extends CreatureAI {
	public Ai_instance_guard_hunter(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait(double blendTime) {
		...
		if(Rnd.getChance(90)) {
			if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, false, object -> getTargetCharacterKey(object) != 60134 && getTargetCharacterKey(object) != 60135 && getTargetCharacterKey(object) != 60136 && getTargetCharacterKey(object) != 60137 && getTargetCharacterKey(object) != 60140 && getTargetCharacterKey(object) != 60141 && getTargetCharacterKey(object) != 60142 && getTargetCharacterKey(object) != 60143 && getDistanceToTarget(object, false) >= 0 && getDistanceToTarget(object, false) <= 1000 && getTargetHp(object) > 0 && isCreatureVisible(object, false))) {
	...
	public EAiHandlerResult HandleReleased(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Damage_Release(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
