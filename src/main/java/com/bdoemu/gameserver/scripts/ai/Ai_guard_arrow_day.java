...
@IAIName("guard_arrow_day")
public class Ai_guard_arrow_day extends CreatureAI {
	public Ai_guard_arrow_day(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) > 1000 && target != null && getTargetHp(target) > 0) {
			if (changeState(state -> Move_Chaser(0.3)))
				return;
		}
		if (target != null && (getDistanceToTarget(target, false) >= 0 && getDistanceToTarget(target, false) <= 1500) && target != null && getTargetHp(target) > 0) {
			if(Rnd.getChance(50)) {
