...
@IAIName("itemnoviceancientbird_boss")
public class Ai_itemnoviceancientbird_boss extends CreatureAI {
	public Ai_itemnoviceancientbird_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Move_Return(double blendTime) {
		...
		setVariable(0xFA9DA674L /*_isBattleMode*/, 0);
		doAction(1317399622L /*RETURN_ING*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 0, 0, false, ENaviType.ground, () -> {
			setVariable(0x96CB9F57L /*_Summon_StartTime*/, getTime());
	...
	protected void Move_Chaser(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) >= 1000 && target != null && getDistanceToTarget(target) < 2000) {
			if (changeState(state -> Attack_Normal5_Short(0.3)))
				return;
		}
	...
	protected void Attack_Normal5_Short(double blendTime) {
		...
		setVariable(0x6E0E85B9L /*_AttackCount*/, getVariable(0x6E0E85B9L /*_AttackCount*/) + 1);
		doAction(2529594218L /*ATTACK_NORMAL5_SHORT_STR*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Normal5_Short_ING(blendTime)));
	}

	protected void Attack_Normal5_Short_ING(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x2A6192E1L /*Attack_Normal5_Short_ING*/);
		doAction(2037973108L /*ATTACK_NORMAL5_SHORT_ING*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Normal5_Short_END(blendTime)));
	}

	protected void Attack_Normal5_Short_END(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xD9E6409BL /*Attack_Normal5_Short_END*/);
		doAction(505908789L /*ATTACK_NORMAL5_SHORT_END*/, blendTime, onDoActionEnd -> scheduleState(state -> Attack_End_Logic(blendTime), 3000));
	}
	...
	protected void Attack_Normal5_Long(double blendTime) {
		...
		setVariable(0x6E0E85B9L /*_AttackCount*/, getVariable(0x6E0E85B9L /*_AttackCount*/) + 1);
		doAction(1003535884L /*ATTACK_NORMAL5_STR*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Normal5_ING(blendTime)));
	}

	protected void Attack_Normal5_ING(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x2750AE29L /*Attack_Normal5_ING*/);
		doAction(38391078L /*ATTACK_NORMAL5_ING*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Normal5_END(blendTime)));
	}

	protected void Attack_Normal5_END(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xF3DC678FL /*Attack_Normal5_END*/);
		doAction(4121062390L /*ATTACK_NORMAL5_END*/, blendTime, onDoActionEnd -> scheduleState(state -> Attack_End_Logic(blendTime), 3000));
	}
