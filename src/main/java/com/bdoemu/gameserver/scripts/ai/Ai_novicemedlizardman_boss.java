...
@IAIName("novicemedlizardman_boss")
public class Ai_novicemedlizardman_boss extends CreatureAI {
	public Ai_novicemedlizardman_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, 0);
	...
	protected void Move_Return(double blendTime) {
		...
		setVariable(0xEBAA9AEAL /*_isRage*/, 0);
		setVariable(0xC766911CL /*_isAirFloatCount*/, 0);
		doAction(3208508703L /*CHASER_RUN*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 0, 0, false, ENaviType.ground, () -> {
			setVariable(0x96CB9F57L /*_Summon_StartTime*/, getTime());
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0xEBAA9AEAL /*_isRage*/) == 1 && target != null && getDistanceToTarget(target) < 700 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if (changeState(state -> Attack_Combo(0.3)))
				return;
		}
		if (getVariable(0xEBAA9AEAL /*_isRage*/) == 0 && getVariable(0x3F487035L /*_Hp*/) <= 50) {
			if (changeState(state -> Attack_Rage(0.3)))
				return;
		}
		if (getVariable(0xC766911CL /*_isAirFloatCount*/) <= 0 && target != null && getDistanceToTarget(target) < 250 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if(Rnd.getChance(25)) {
				if (changeState(state -> Attack_AirFloat(0.3)))
					return;
			}
		}
		if (getVariable(0xEBAA9AEAL /*_isRage*/) == 1 && target != null && getDistanceToTarget(target) < 700 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
		...
		if (getVariable(0xEBAA9AEAL /*_isRage*/) == 0 && target != null && getDistanceToTarget(target) < 700 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if(Rnd.getChance(15)) {
				if (changeState(state -> Attack_Charge1(0.3)))
					return;
			}
		}
		if (target != null && (getDistanceToTarget(target, false) >= 0 && getDistanceToTarget(target, false) <= 900) && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if(Rnd.getChance(15)) {
		...
		if (target != null && getDistanceToTarget(target) >= 800 && getDistanceToSpawn() < 3000) {
			if(Rnd.getChance(35)) {
				if (changeState(state -> Attack_Chase_Run(0.3)))
					return;
			}
		}
	...
	protected void Attack_Normal1(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, getVariable(0xC766911CL /*_isAirFloatCount*/) - 1);
	...
	protected void Attack_Charge1(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, getVariable(0xC766911CL /*_isAirFloatCount*/) - 1);
		doAction(643630764L /*BATTLE_CHARGE_ATTACK1*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
	...
	protected void Attack_Charge2(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, getVariable(0xC766911CL /*_isAirFloatCount*/) - 1);
		doAction(1144170621L /*BATTLE_CHARGE_ATTACK2*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}

	protected void Attack_Chase_Run(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.chase);
		setState(0xBF6358B9L /*Attack_Chase_Run*/);
		if (isTargetLost()) {
			if (changeState(state -> Logic(blendTime)))
				return;
		}
		...
		doAction(70845698L /*CHASER_RUN_FAST*/, blendTime, onDoActionEnd -> chase(10, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
				if (changeState(state -> FailFindPath_Logic(0.3)))
					return true;
			...
			return false;
		}, onExit -> scheduleState(state -> Attack_Chase(blendTime), 100)));
	}
	...
	protected void Attack_Chase(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, getVariable(0xC766911CL /*_isAirFloatCount*/) - 1);
		doAction(1605041147L /*BATTLE_CHASE_ATTACK*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
	...
	protected void Attack_Jump(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, getVariable(0xC766911CL /*_isAirFloatCount*/) - 1);
		doAction(2197303760L /*BATTLE_JUMP_ATTACK*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
	...
	protected void Attack_Combo(double blendTime) {
		...
		setVariable(0xC766911CL /*_isAirFloatCount*/, getVariable(0xC766911CL /*_isAirFloatCount*/) - 1);
		doAction(1703514227L /*BATTLE_ATTACK_COM*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}

	protected void Attack_AirFloat(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x672444B4L /*Attack_AirFloat*/);
		setVariable(0xC766911CL /*_isAirFloatCount*/, 2);
		doAction(2923256539L /*BATTLE_ATTACK_AIRFLOAT*/, blendTime, onDoActionEnd -> changeState(state -> AirFloat_Logic(blendTime)));
	}

	protected void AirFloat_Logic(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.none);
		setState(0x74B548B9L /*AirFloat_Logic*/);
		if (target != null && getDistanceToTarget(target) < 400 && target != null && getAngleToTarget(target) > -25 && target != null && getAngleToTarget(target) < 25 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
			if (changeState(state -> Attack_AirFloat_B(0.3)))
				return;
		}
		changeState(state -> Attack_AirFloat_A(blendTime));
	}

	protected void Attack_AirFloat_A(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xCA47A37L /*Attack_AirFloat_A*/);
		doAction(2998079582L /*BATTLE_ATTACK_AIRSMASH_A*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}

	protected void Attack_AirFloat_B(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x3319341DL /*Attack_AirFloat_B*/);
		doAction(4194802761L /*BATTLE_ATTACK_AIRSMASH_B*/, blendTime, onDoActionEnd -> changeState(state -> Attack_End_Logic(blendTime)));
	}
