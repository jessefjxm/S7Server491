...
@IAIName("party_leader_perina")
public class Ai_party_leader_perina extends CreatureAI {
	public Ai_party_leader_perina(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait(double blendTime) {
		...
		if (getPartyMembersCount()>= 1 && getPartyMembersCount()< 4) {
			if (changeState(state -> Summon_Pericana(blendTime)))
				return;
		}
	...
	protected void Stop(double blendTime) {
		...
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Wait(blendTime), 14000));
	}
	...
	protected void Go1(double blendTime) {
		...
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.WaypointVariable, getVariableArray(0x11AADB03L /*_WaypointValue2*/), ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> Stop1(blendTime), 2000)));
	}
