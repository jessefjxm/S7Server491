...
@IAIName("npc_abun_tocrio_roaming")
public class Ai_npc_abun_tocrio_roaming extends CreatureAI {
	public Ai_npc_abun_tocrio_roaming(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait(double blendTime) {
		...
		if (changeState(state -> Select_CrioTown_Position(blendTime)))
			return;
	...
	protected void Wait_2(double blendTime) {
		...
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Select_CrioTown_Position(blendTime), 10000));
	}
	...
	protected void Select_CrioTown_Position(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xA1CCEB7BL /*Select_CrioTown_Position*/);
		setVariable(0x2377F21EL /*_Waypointpositoin*/, getRandom(40));
		if (getVariable(0x2377F21EL /*_Waypointpositoin*/) <= 1) {
		...
		if (changeState(state -> Move_CrioTown_Position(blendTime)))
			return;
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Select_CrioTown_Position(blendTime), 5000 + Rnd.get(-500,500)));
	}

	protected void Move_CrioTown_Position(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.walk);
		setState(0xAF6C2A56L /*Move_CrioTown_Position*/);
		if(Rnd.getChance(10)) {
			if (changeState(state -> Move_Random_CrioTown(blendTime)))
				return;
		}
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.WaypointVariable, getVariableArray(0xACD660AFL /*WaypointValue*/), ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
	...
	protected void Move_Random_CrioTown(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.walk);
		setState(0xCAABAD6DL /*Move_Random_CrioTown*/);
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 500, 700, true, ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0 && getVariable(0xFDC61BCCL /*_FailFindPathCount*/) >= 3) {
				if (changeState(state -> Die_Wait(0.3)))
			...
			return false;
		}, onExit -> scheduleState(state -> Wait_for_Talk_CrioTown_Position(blendTime), 15000)));
	}

	protected void Wait_for_Talk_CrioTown_Position(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x96B4B1EDL /*Wait_for_Talk_CrioTown_Position*/);
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Move_CrioTown_Position(blendTime), 10000));
	}
