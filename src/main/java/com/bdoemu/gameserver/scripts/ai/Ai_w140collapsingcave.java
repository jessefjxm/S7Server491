...
@IAIName("w140collapsingcave")
public class Ai_w140collapsingcave extends CreatureAI {
	public Ai_w140collapsingcave(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Down(double blendTime) {
		...
		doAction(2649688712L /*DOWN*/, blendTime, onDoActionEnd -> scheduleState(state -> Down1(blendTime), 1000));
	}

	protected void Down1(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x7D52675DL /*Down1*/);
		doAction(2649688712L /*DOWN*/, blendTime, onDoActionEnd -> scheduleState(state -> Down1(blendTime), 1000));
	}
