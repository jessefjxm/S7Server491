...
@IAIName("m0126_hunt_deer")
public class Ai_m0126_hunt_deer extends CreatureAI {
	public Ai_m0126_hunt_deer(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait(double blendTime) {
		...
		if (getVariable(0x64490D98L /*AI_RandomMove*/) == 1 && getVariable(0xB0A90ED8L /*_RunAwayCount*/) == 0) {
			if(getCallCount() == 5) {
		...
		if (getVariable(0xB0A90ED8L /*_RunAwayCount*/) >= 1) {
			if(getCallCount() == 200) {
				if (changeState(state -> Move_Return2(0.4)))
					return;
			}
		}
	...
	protected void Search_Enemy(double blendTime) {
		...
		doAction(11967395L /*SEARCH_ENEMY*/, blendTime, onDoActionEnd -> scheduleState(state -> Run_Random(blendTime), 1000));
	}
	...
	protected void Move_Return(double blendTime) {
		...
		doAction(3326922924L /*MOVE_CHASER*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 0, 1, false, ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
	...
	protected void FailFindPath(double blendTime) {
		...
		clearAggro(true);
		doAction(2407452960L /*Wait*/, blendTime, onDoActionEnd -> scheduleState(state -> Move_Return2(blendTime), 1000));
	}
	...
	protected void Run_Random(double blendTime) {
		...
		doAction(3326922924L /*MOVE_CHASER*/, blendTime, onDoActionEnd -> escape(2000, () -> {
			...
			return false;
		}, onExit -> scheduleState(state -> AggroClear_Run_Random(blendTime), 1000)));
	}
