...
@IAIName("party_plunderleader_ghost")
public class Ai_party_plunderleader_ghost extends CreatureAI {
	public Ai_party_plunderleader_ghost(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Summon_Wait(double blendTime) {
		...
		doAction(3602854557L /*SUMMON_WAIT*/, blendTime, onDoActionEnd -> {
			if (getPartyMembersCount()<= 3) {
				if(Rnd.getChance(20)) {
	...
	protected void Loop_Wait(double blendTime) {
		...
		clearAggro(true);
		getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().HandleTeleportToParent(getActor(), null));
	...
	protected void Wait(double blendTime) {
		...
		if (getPartyMembersCount()> 3 && getVariable(0xABA96D52L /*AI_WayPointLoopCount*/) >= 1) {
			if(getCallCount() == 300) {
	...
	protected void Loop_Logic(double blendTime) {
		...
		changeState(state -> MovePoint(blendTime));
	}
