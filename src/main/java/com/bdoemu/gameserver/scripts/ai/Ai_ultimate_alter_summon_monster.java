...
@IAIName("ultimate_alter_summon_monster")
public class Ai_ultimate_alter_summon_monster extends CreatureAI {
	public Ai_ultimate_alter_summon_monster(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		setVariable(0x684FFA00L /*_BossCount*/, 0);
	...
	protected void Wait_Rotate(double blendTime) {
		...
		doAction(3401480022L /*WAIT2*/, blendTime, onDoActionEnd -> scheduleState(state -> Wait_Rotate(blendTime), 1000));
	}
	...
	protected void NextStageLogic(double blendTime) {
		...
		getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().ChildAllDie(getActor(), null));
		if (checkInstanceTeamNo()) {
			getObjects(EAIFindTargetType.Monster, object -> getDistanceToTarget(object, false) >= 0 && getDistanceToTarget(object, false) <= 1000 && getTargetCharacterKey(object) == 27284).forEach(consumer -> consumer.getAi().NextStage(getActor(), null));
		}
	...
	protected void Summon_Wave2_B2(double blendTime) {
		...
		doAction(2644087142L /*BATTLE_SUMMON_WAVE2_B2*/, blendTime, onDoActionEnd -> changeState(state -> Wait(blendTime)));
	}
	...
	protected void Summon_Wave3_A1(double blendTime) {
		...
		doAction(3637361715L /*BATTLE_SUMMON_WAVE3_A1*/, blendTime, onDoActionEnd -> changeState(state -> Wait(blendTime)));
	}
	...
	protected void Summon_Wave3_A2(double blendTime) {
		...
		doAction(886505864L /*BATTLE_SUMMON_WAVE3_A2*/, blendTime, onDoActionEnd -> changeState(state -> Wait(blendTime)));
	}
	...
	protected void Summon_Wave4_End(double blendTime) {
		...
		getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().ChildAllDie(getActor(), null));
		doAction(1915380717L /*BATTLE_SUMMON_WAVE4_END*/, blendTime, onDoActionEnd -> changeState(state -> Wait(blendTime)));
	}
	...
	protected void Turn_Undead_Logic(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.none);
		setState(0xA54230AFL /*Turn_Undead_Logic*/);
		instanceNotify(EChatNoticeType.Defence, "GAME", "LUA_DEFENCE_ALTER_4STAGE_BOSS_END2");
		getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().ChildAllDie(getActor(), null));
		if (checkInstanceTeamNo()) {
			getObjects(EAIFindTargetType.Monster, object -> getDistanceToTarget(object, false) >= 0 && getDistanceToTarget(object, false) <= 1000 && getTargetCharacterKey(object) == 27284).forEach(consumer -> consumer.getAi().NextStage(getActor(), null));
		}
	...
	@Override
	public EAiHandlerResult Wave2_B2(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Summon_Wave2_B2(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
	...
	@Override
	public EAiHandlerResult Wave3_A1(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Summon_Wave3_A1(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}

	@Override
	public EAiHandlerResult Wave3_A2(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Summon_Wave3_A2(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
	...
	@Override
	public EAiHandlerResult Wave4_END(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Summon_Wave4_End(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}

	@Override
	public EAiHandlerResult SkeletonKing(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
	...
	@Override
	public EAiHandlerResult Turn_Undead(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Turn_Undead_Logic(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
	...
	@Override
	public EAiHandlerResult NextMove(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
