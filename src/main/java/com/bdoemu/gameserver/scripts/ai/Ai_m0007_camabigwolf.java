...
@IAIName("m0007_camabigwolf")
public class Ai_m0007_camabigwolf extends CreatureAI {
	public Ai_m0007_camabigwolf(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Search_Enemy(double blendTime) {
		...
		if (getVariable(0x870CD143L /*_IsPartyMember*/) == 0) {
			getObjects(EAIFindTargetType.Monster, object -> getTargetCharacterKey(object) == 21523 && getDistanceToTarget(object) < 5000).forEach(consumer -> consumer.getAi()._helpme(getActor(), null));
		}
	...
	protected void TargetLost(double blendTime) {
		...
		clearAggro(true);
		if (getVariable(0xFA9DA674L /*_isBattleMode*/) == 1 && getVariable(0x870CD143L /*_IsPartyMember*/) == 0) {
	...
	protected void Attack_Logic(double blendTime) {
		...
		if (target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) < 350) {
			if(Rnd.getChance(20)) {
		...
		if (target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) < 100) {
			if (changeState(state -> Battle_Attack1(0.3)))
				return;
		}
		if (target != null && getDistanceToTarget(target) > 130 && target != null && getDistanceToTarget(target) <= 400) {
