...
@IAIName("elephantbaby_ride")
public class Ai_elephantbaby_ride extends CreatureAI {
	public Ai_elephantbaby_ride(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Parking_Wait(double blendTime) {
		...
		if (getSelfInventoryWeight() >= 80) {
			if (changeState(state -> Parking_Wait_Heavy(blendTime)))
				return;
		}
		doAction(1647787509L /*parking_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> TerminateState(blendTime), 5000));
	}

	protected void Parking_Wait_Heavy(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x16FFC723L /*Parking_Wait_Heavy*/);
		if (getSelfInventoryWeight() < 80) {
			if (changeState(state -> Parking_Wait(blendTime)))
				return;
		}
		doAction(2164678255L /*parking_WAIT_HEAVY*/, blendTime, onDoActionEnd -> scheduleState(state -> TerminateState(blendTime), 5000));
	}
	...
	protected void ChaseOwner_MoveLv3(double blendTime) {
		...
		doAction(3606685177L /*MOVE_LV3_ING*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPosition, 0, 0, 0, 0, 0, false, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> ChaseOwner_MoveLv3(blendTime), 300)));
	}

	protected void ChaseOwner_MoveLv3_2(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.walk);
		setState(0x7970C9FEL /*ChaseOwner_MoveLv3_2*/);
		setVariable(0xAB16882DL /*isUnderWater*/, (getCurrentPos_NaviType()==getNaviTypeStringToEnum("under_water_ground") ? 1 : 0));
		setVariable(0x9617E562L /*isWater*/, (getCurrentPos_NaviType()==getNaviTypeStringToEnum("water") ? 1 : 0));
		setVariable(0xCBEEF8C7L /*_ownerDistance*/, getDistanceToOwner());
		if (getVariable(0xAB16882DL /*isUnderWater*/) == 1) {
		...
		doAction(2905894929L /*MOVE_LV3_ING_2*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPosition, 0, 0, 0, 0, 0, false, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> ChaseOwner_MoveLv3_2(blendTime), 300)));
	}
	...
	protected void ChaseOwner_MoveLv4(double blendTime) {
		...
		doAction(1772021153L /*MOVE_LV4_ING*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPosition, 0, 0, 0, 0, 0, false, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> ChaseOwner_MoveLv4(blendTime), 100)));
	}

	protected void ChaseOwner_MoveLv4_2(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.walk);
		setState(0xBB0B8421L /*ChaseOwner_MoveLv4_2*/);
		setVariable(0xAB16882DL /*isUnderWater*/, (getCurrentPos_NaviType()==getNaviTypeStringToEnum("under_water_ground") ? 1 : 0));
		setVariable(0x9617E562L /*isWater*/, (getCurrentPos_NaviType()==getNaviTypeStringToEnum("water") ? 1 : 0));
		setVariable(0xCBEEF8C7L /*_ownerDistance*/, getDistanceToOwner());
		if (getVariable(0xAB16882DL /*isUnderWater*/) == 1) {
		...
		doAction(2362935870L /*MOVE_LV4_ING_2*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.OwnerPosition, 0, 0, 0, 0, 0, false, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> ChaseOwner_MoveLv4_2(blendTime), 100)));
	}
	...
	protected void ChaseOwner_Stop(double blendTime) {
		...
		doAction(799326965L /*MOVE_SLOW_STOP_SHORT*/, blendTime, onDoActionEnd -> changeState(state -> Wait(blendTime)));
	}
	...
	protected void MovingStop_Lv4(double blendTime) {
		...
		doAction(799326965L /*MOVE_SLOW_STOP_SHORT*/, blendTime, onDoActionEnd -> changeState(state -> Parking_Wait(blendTime)));
	}
	...
	protected void Damage_KnockDown(double blendTime) {
		...
		doAction(158462624L /*KNOCKDOWN_ELEPHANTBABY*/, blendTime, onDoActionEnd -> scheduleState(state -> Damage_KnockDown_Wait(blendTime), 1000));
	}
	...
	protected void Damage_KnockDown_At_Moving(double blendTime) {
		...
		doAction(3988151087L /*KNOCKDOWN_ELEPHANTBABY_AT_MOVING*/, blendTime, onDoActionEnd -> scheduleState(state -> Damage_KnockDown_Wait(blendTime), 1000));
	}
	...
	protected void Damage_KnockDown_Wait(double blendTime) {
		...
		doAction(3089467598L /*KNOCKDOWN_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Riding_Wait(blendTime), 7000));
	}
	...
	protected void NaturalDeath(double blendTime) {
		...
		doAction(2544805566L /*DIE*/, blendTime, onDoActionEnd -> scheduleState(state -> NaturalDeath(blendTime), 2000));
	}
	...
	protected void UnderWater_KnockDown(double blendTime) {
		...
		doAction(158462624L /*KNOCKDOWN_ELEPHANTBABY*/, blendTime, onDoActionEnd -> scheduleState(state -> ChaseOwner_MoveLv2(blendTime), 1000));
	}
	...
	public EAiHandlerResult HandleRideEnd(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getVariable(0x42A6EC2DL /*_isRunning*/) == 1 && getState() != 0x25EE997L /*Damage_KnockDown_At_Moving*/ && getState() != 0x69E1FC3AL /*Damage_KnockDown*/ && getState() != 0xB669FB51L /*Damage_KnockBack_At_Moving*/ && getState() != 0xBF725BC4L /*Damage_KnockBack*/ && getState() != 0xBF7E038EL /*UnderWater_KnockDown*/) {
			if (changeState(state -> MovingStop_Lv2(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		if (getVariable(0x940229E0L /*_isState*/) != 2 && getState() != 0x25EE997L /*Damage_KnockDown_At_Moving*/ && getState() != 0x69E1FC3AL /*Damage_KnockDown*/ && getState() != 0xB669FB51L /*Damage_KnockBack_At_Moving*/ && getState() != 0xBF725BC4L /*Damage_KnockBack*/ && getState() != 0xBF7E038EL /*UnderWater_KnockDown*/) {
			if (changeState(state -> Wait(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		if (getState() != 0x25EE997L /*Damage_KnockDown_At_Moving*/ && getState() != 0x69E1FC3AL /*Damage_KnockDown*/ && getState() != 0xB669FB51L /*Damage_KnockBack_At_Moving*/ && getState() != 0xBF725BC4L /*Damage_KnockBack*/ && getState() != 0xBF7E038EL /*UnderWater_KnockDown*/) {
			if (changeState(state -> Parking_Wait(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
