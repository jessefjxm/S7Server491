...
@IAIName("ru_novicemudman_boss")
public class Ai_ru_novicemudman_boss extends CreatureAI {
	public Ai_ru_novicemudman_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		setVariable(0xBD142893L /*_Attack_Bomb_Count*/, 10);
	...
	protected void CheckSpawn_Logic(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, true, object -> getDistanceToTarget(object) < 4000 && getTargetCharacterKey(object) == 23055)) {
	...
	protected void Notifier_Logic(double blendTime) {
		...
		worldNotify(EChatNoticeType.Nuberu, "GAME", "LUA_MUDMAN_SPAWN");
	...
	protected void Delete_Die_Logic(double blendTime) {
		...
		worldNotify(EChatNoticeType.Nuberu, "GAME", "LUA_MUDMAN_TIMEOUT");
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0x3F487035L /*_HP*/) < 70 && getVariable(0xBD142893L /*_Attack_Bomb_Count*/) == 10) {
			if (changeState(state -> Attack_Boom(0.3)))
				return;
		}
		if (getVariable(0x3F487035L /*_HP*/) < 20 && getVariable(0xBD142893L /*_Attack_Bomb_Count*/) == 1) {
			if (changeState(state -> Attack_Boom(0.3)))
				return;
		}
		...
		if (target != null && getDistanceToTarget(target) < 650 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/) && target != null && isCreatureVisible(target, false)) {
			if(Rnd.getChance(5)) {
				if (changeState(state -> Attack_MudWave(0.3)))
					return;
			}
		}
	...
	protected void Attack_MudWave(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xC62816AL /*Attack_MudWave*/);
		doAction(4147109407L /*ATTACK_MUDWAVE*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}

	protected void Attack_Boom(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x41A0A4D5L /*Attack_Boom*/);
		setVariable(0xBD142893L /*_Attack_Bomb_Count*/, getVariable(0xBD142893L /*_Attack_Bomb_Count*/) - 1);
		doAction(3210984924L /*ATTACK_BOOM*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Boom_Hide(blendTime)));
	}

	protected void Attack_Boom_Hide(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x4CBCDFEEL /*Attack_Boom_Hide*/);
		if (getPartyMembersCount()== 0) {
			if (changeState(state -> Attack_Boom_End(blendTime)))
				return;
		}
		doAction(53652545L /*ATTACK_BOOM_HIDE*/, blendTime, onDoActionEnd -> changeState(state -> Attack_Boom_Hide(blendTime)));
	}

	protected void Attack_Boom_End(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x45D59BFCL /*Attack_Boom_End*/);
		doAction(924762676L /*ATTACK_BOOM_END*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
