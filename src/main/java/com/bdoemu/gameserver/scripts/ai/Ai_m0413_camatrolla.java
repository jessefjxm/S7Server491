...
@IAIName("m0413_camatrolla")
public class Ai_m0413_camatrolla extends CreatureAI {
	public Ai_m0413_camatrolla(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) > 400 && target != null && getDistanceToTarget(target) <= 1000) {
			if(Rnd.getChance(40)) {
	...
	protected void Move_Back_Ing(double blendTime) {
		...
		doAction(407722945L /*MOVE_BACK_ING*/, blendTime, onDoActionEnd -> escape(800, () -> {
			return false;
	...
	protected void Battle_Attack1(double blendTime) {
		...
		doAction(31162842L /*BATTLE_ATTACK1*/, blendTime, onDoActionEnd -> {
			if(Rnd.getChance(10)) {
				if (changeState(state -> Move_Back_Ing(0.2)))
					return;
			}
			if (changeState(state -> Battle_Wait(0.4)))
				return;
