...
@IAIName("novicemedderserthorse_boss")
public class Ai_novicemedderserthorse_boss extends CreatureAI {
	public Ai_novicemedderserthorse_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0x3F487035L /*_HP*/) < 50) {
			if (changeState(state -> Die_Summon(0.3)))
				return;
		}
	...
	protected void Die_Summon(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x1B00F79BL /*Die_Summon*/);
		doAction(2050218913L /*DIE_SUMMON*/, blendTime, onDoActionEnd -> changeState(state -> Die_Summon_A(blendTime)));
	}

	protected void Die_Summon_A(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.dead);
		setState(0x9F3B64BCL /*Die_Summon_A*/);
		doAction(2513337136L /*DIE_SUMMON_A*/, blendTime, onDoActionEnd -> scheduleState(state -> Die_Summon_A(blendTime), 1000));
	}
