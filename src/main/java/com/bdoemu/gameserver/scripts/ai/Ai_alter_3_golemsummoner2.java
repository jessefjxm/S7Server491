...
@IAIName("alter_3_golemsummoner2")
public class Ai_alter_3_golemsummoner2 extends CreatureAI {
	public Ai_alter_3_golemsummoner2(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (checkInstanceTeamNo() && target != null && getTargetCharacterKey(target) == 27340) {
	...
	protected void SummonLogic(double blendTime) {
		...
		setVariable(0x99703876L /*_SummonCount*/, getVariable(0x99703876L /*_SummonCount*/) + 1);
							if (getVariable(0x4B5B7DBBL /*_isNumber*/) == 1 && getVariable(0x99703876L /*_SummonCount*/) > 3) {
	...
	protected void Battle_Wait_Junk(double blendTime) {
		...
		doAction(1160482186L /*BATTLE_WAIT_GREENJUNK*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
	...
	protected void Battle_Wait_Junk2(double blendTime) {
		...
		doAction(1160482186L /*BATTLE_WAIT_GREENJUNK*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait1(blendTime)));
	}
	...
	protected void Paralysis_Logic(double blendTime) {
		...
		if (checkInstanceTeamNo()) {
			getObjects(EAIFindTargetType.Monster, object -> getDistanceToTarget(object) < 500 && getTargetCharacterKey(object) == 27340).forEach(consumer -> consumer.getAi().Golem_Eater(getActor(), null));
		}
	...
	protected void Paralysis(double blendTime) {
		...
		instanceNotify(EChatNoticeType.Defence, "GAME", "LUA_DEFENCE_ALTER_DAMAGE_3STAGE_SUMMONBREAK_GREEN");
	...
	protected void RecoveryNotifier(double blendTime) {
		...
		instanceNotify(EChatNoticeType.Defence, "GAME", "LUA_DEFENCE_ALTER_DAMAGE_3STAGE_GOLEMSUMMONER_GREEN");
	...
	protected void Die_Logic(double blendTime) {
		...
		instanceNotify(EChatNoticeType.Defence, "GAME", "LUA_DEFENCE_ALTER_DAMAGE_3STAGE_SUMMONBREAK_GREEN2");
