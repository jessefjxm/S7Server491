...
@IAIName("w9000_1_minion")
public class Ai_w9000_1_minion extends CreatureAI {
	public Ai_w9000_1_minion(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Move_Line0(double blendTime) {
		...
		doAction(633942026L /*MOVE_WALK*/, blendTime, onDoActionEnd -> moveToWaypoint("battlefield", "w10000_team1_route0_8", ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> set_Stance(blendTime), 1000)));
	}
	...
	protected void Move_Line2(double blendTime) {
		...
		doAction(633942026L /*MOVE_WALK*/, blendTime, onDoActionEnd -> moveToWaypoint("battlefield", "w10000_team1_route2_7", ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> set_Stance(blendTime), 1000)));
	}
	...
	public EAiHandlerResult HandleEvent1(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Move_Line0(0.1)))
	...
	public EAiHandlerResult HandleEvent2(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Move_Line1(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
	...
	public EAiHandlerResult HandleEvent3(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Move_Line2(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
		return EAiHandlerResult.BYPASS;
	}
