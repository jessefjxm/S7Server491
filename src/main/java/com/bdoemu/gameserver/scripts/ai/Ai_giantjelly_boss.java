...
@IAIName("giantjelly_boss")
public class Ai_giantjelly_boss extends CreatureAI {
	public Ai_giantjelly_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void CheckSpawn_Logic(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, true, object -> getDistanceToTarget(object) < 4000 && getTargetCharacterKey(object) == 23913)) {
	...
	protected void Notifier_Logic(double blendTime) {
		...
		worldNotify(EChatNoticeType.Kzarka, "GAME", "LUA_CZAKA_SQAWN");
	...
	protected void Waiting(double blendTime) {
		...
		doAction(3401480022L /*WAIT2*/, blendTime, onDoActionEnd -> {
			if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, true, object -> getDistanceToTarget(object) < 4000 && getTargetCharacterKey(object) == 23913)) {
				if (changeState(state -> Waiting(0.3)))
					return;
			}
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0x3F487035L /*_HP*/) < 85 && getVariable(0x418BA373L /*_BreathAttackCount*/) == 0) {
			if (changeState(state -> Attack_Breath(blendTime)))
				return;
		}
		if (getVariable(0x3F487035L /*_HP*/) < 60 && getVariable(0x418BA373L /*_BreathAttackCount*/) == 1) {
			if (changeState(state -> Attack_Quake_Rage(blendTime)))
				return;
		}
		if (getVariable(0x3F487035L /*_HP*/) < 35 && getVariable(0x418BA373L /*_BreathAttackCount*/) == 2) {
			if (changeState(state -> Attack_Quake_Rage(blendTime)))
				return;
		}
		if (getVariable(0x3F487035L /*_HP*/) < 10 && getVariable(0x418BA373L /*_BreathAttackCount*/) == 3) {
			if (changeState(state -> Attack_Quake_Rage(blendTime)))
				return;
		}
	...
	protected void Attack_Quake_Rage(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xD37D1F7EL /*Attack_Quake_Rage*/);
		setVariable(0x418BA373L /*_BreathAttackCount*/, getVariable(0x418BA373L /*_BreathAttackCount*/) + 1);
		setVariable(0x6E0E85B9L /*_AttackCount*/, 0);
		doAction(2990545219L /*ATTACK_QUAKE_RAGE*/, blendTime, onDoActionEnd -> {
			if (getVariable(0x418BA373L /*_BreathAttackCount*/) == 2) {
				if (changeState(state -> Attack_Quake_Rage3(blendTime)))
					return;
			}
			if (getVariable(0x418BA373L /*_BreathAttackCount*/) == 4) {
				if (changeState(state -> Attack_Quake_Rage3(blendTime)))
					return;
			}
			changeState(state -> Attack_Breath_Rage(blendTime));
		});
	}

	protected void Attack_Quake_Rage3(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xC57DD49CL /*Attack_Quake_Rage3*/);
		doAction(2415259847L /*ATTACK_QUAKE_RAGE3*/, blendTime, onDoActionEnd -> changeState(state -> Change_Target(blendTime)));
	}
	...
	protected void Attack_Breath_Rage(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xDC2F217FL /*Attack_Breath_Rage*/);
		if (isTargetLost()) {
			if (changeState(state -> Logic(blendTime)))
				return;
		}
