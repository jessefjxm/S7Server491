...
@IAIName("npc_ship_merchant")
public class Ai_npc_ship_merchant extends CreatureAI {
	public Ai_npc_ship_merchant(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void FirstGo(double blendTime) {
		...
		doAction(2337397317L /*MOVE*/, blendTime, onDoActionEnd -> moveToWaypoint("carriage", "merchant_ship_loop_1 merchant_ship_loop_29", ENaviType.ground, () -> {
			return false;
	...
	protected void Go_Start(double blendTime) {
		...
		doAction(4060078600L /*MOVING_START*/, blendTime, onDoActionEnd -> scheduleState(state -> Go(blendTime), 50000));
	}
	...
	protected void End(double blendTime) {
		...
		doAction(155621870L /*MOVING_END*/, blendTime, onDoActionEnd -> scheduleState(state -> Go_Start(blendTime), 50000));
	}
