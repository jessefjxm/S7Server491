...
@IAIName("summon_turretbuilding")
public class Ai_summon_turretbuilding extends CreatureAI {
	public Ai_summon_turretbuilding(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Working_Str(double blendTime) {
		...
		useSkill(40407, 1, false, EAIFindTargetType.Self, object -> true);
	...
	protected void Wait_Logic(double blendTime) {
		...
		if (findTarget(EAIFindTargetType.Character, EAIFindType.normal, true, object -> getTargetCharacterKey(object) == 908 && getDistanceToTarget(object) < 100)) {
	...
	protected void Die(double blendTime) {
		...
		getObjects(EAIFindTargetType.Character, object -> getDistanceToTarget(object) < 100 && getTargetCharacterKey(object) == 908).forEach(consumer -> consumer.getAi()._DestroyedBase(getActor(), null));
