...
@IAIName("summon_hunnishbarricade")
public class Ai_summon_hunnishbarricade extends CreatureAI {
	public Ai_summon_hunnishbarricade(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Before_Die(double blendTime) {
		...
		doAction(1174134199L /*BEFORE_DIE*/, blendTime, onDoActionEnd -> {
			// Missing AI Handler: ReFindTarget : send_command()
			scheduleState(state -> Die(blendTime), 500);
		});
	}
}
