...
@IAIName("hugewhale2_boss")
public class Ai_hugewhale2_boss extends CreatureAI {
	public Ai_hugewhale2_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) > 0 && target != null && getDistanceToTarget(target) < 630) {
			if (changeState(state -> Move_Back(0.3)))
				return;
		}
		if (target != null && getDistanceToTarget(target) > 650 && target != null && getDistanceToTarget(target) < 900) {
			if(Rnd.getChance(50)) {
		...
		if (target != null && getAngleToTarget(target) <= -20 && target != null && getDistanceToTarget(target) < 650) {
	...
	protected void Move_Chaser(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) < 850) {
			if(Rnd.getChance(30)) {
				if (changeState(state -> Attack_Normal2(0.3)))
					return;
			}
	...
	protected void Attack_Normal1(double blendTime) {
		...
		doAction(1616805723L /*ATTACK_NORMAL1*/, blendTime, onDoActionEnd -> changeState(state -> Move_RunAway(blendTime)));
	}
	...
	protected void Attack_Normal2(double blendTime) {
		...
		doAction(2463627859L /*ATTACK_NORMAL2*/, blendTime, onDoActionEnd -> changeState(state -> Move_RunAway(blendTime)));
	}
	...
	protected void Battle_Move_Random(double blendTime) {
		...
		doAction(3208508703L /*CHASER_RUN*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 5000, 7000, true, ENaviType.ground, () -> {
			return false;
		}, onExit -> scheduleState(state -> Battle_Wait_Delay(blendTime), 1000)));
	}
	...
	protected void Battle_Wait_Delay(double blendTime) {
		...
		doAction(2820327238L /*BATTLE_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Battle_Wait(blendTime), 9000));
	}
