...
@IAIName("kamawooddummy2_boss")
public class Ai_kamawooddummy2_boss extends CreatureAI {
	public Ai_kamawooddummy2_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		setVariable(0x73E18951L /*_SummonDummyCount*/, 0);
		setVariable(0xBF33D5C1L /*_BrokenShieldCount*/, 0);
		setVariable(0x3549AFC5L /*_CombinationAttackCount*/, 0);
		setVariable(0x444DFF4EL /*_isFindPathCompleted*/, 0);
		setVariable(0xFDC61BCCL /*_FailFindPathCount*/, 0);
		doAction(807741651L /*Avoid_Wait*/, blendTime, onDoActionEnd -> scheduleState(state -> Appear(blendTime), 1000));
	}
	...
	protected void Move_Return(double blendTime) {
		...
		clearAggro(true);
		setVariable(0xFDC61BCCL /*_FailFindPathCount*/, 0);
		doAction(1516377807L /*MOVE_BACK*/, blendTime, onDoActionEnd -> escape(0, () -> {
			return false;
		}, onExit -> scheduleState(state -> Move_Return_A(blendTime), 2000)));
	}
	...
	protected void Move_Return_B(double blendTime) {
		...
		clearAggro(true);
		setVariable(0xFDC61BCCL /*_FailFindPathCount*/, 0);
		doAction(1357246757L /*TELEPORT_A*/, blendTime, onDoActionEnd -> recoveryAndReturn(() -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
	...
	protected void Move_Return_C(double blendTime) {
		...
		doTeleport(EAIMoveDestType.OwnerPosition, 300, 0, 0, 0);
		doAction(3090905698L /*TELEPORT_B*/, blendTime, onDoActionEnd -> changeState(state -> Wait(blendTime)));
	}
	...
	protected void Teleport_Logic(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) < 800) {
			if(Rnd.getChance(100)) {
				if (changeState(state -> Teleport_Right(0.2)))
					return;
			}
		}
		changeState(state -> Battle_Wait(blendTime));
	...
	protected void Before_Die(double blendTime) {
		...
		doAction(1174134199L /*BEFORE_DIE*/, blendTime, onDoActionEnd -> {
			getObjects(EAIFindTargetType.Parent, object -> getTargetCharacterKey(object) == 23751).forEach(consumer -> consumer.getAi().Die(getActor(), null));
			scheduleState(state -> Die(blendTime), 1000);
		});
	}
	...
	protected void Disappear(double blendTime) {
		...
		getObjects(EAIFindTargetType.Child, object -> getTargetCharacterKey(object) == 23765).forEach(consumer -> consumer.getAi().Disappear_Dummy(getActor(), null));
		doAction(1903499173L /*DISAPPEAR*/, blendTime, onDoActionEnd -> {
			getObjects(EAIFindTargetType.Parent, object -> getTargetCharacterKey(object) == 23751).forEach(consumer -> consumer.getAi().Revival(getActor(), null));
	...
	@Override
	public EAiHandlerResult Disappear_Dummy(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Disappear(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
	...
	@Override
	public EAiHandlerResult Appear_Dummy(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (changeState(state -> Appear(0.1)))
			return EAiHandlerResult.CHANGE_STATE;
	...
	@Override
	public EAiHandlerResult Teleport(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (target != null && getDistanceToTarget(target) < 800) {
			if (changeState(state -> Teleport_Right(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
	}
