...
@IAIName("npc_chicken_farm")
public class Ai_npc_chicken_farm extends CreatureAI {
	public Ai_npc_chicken_farm(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Wait_1(double blendTime) {
		...
		doAction(1569894142L /*WAIT_1*/, blendTime, onDoActionEnd -> scheduleState(state -> Wait_1(blendTime), 10000));
	}
	...
	protected void FailFindPath_Logic_1(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x47322928L /*FailFindPath_Logic_1*/);
		setVariable(0xFDC61BCCL /*_FailFindPathCount*/, getVariable(0xFDC61BCCL /*_FailFindPathCount*/) + 1);
		if (getVariable(0xFDC61BCCL /*_FailFindPathCount*/) >= 3) {
			if (changeState(state -> FailFindPath_1(0.3)))
				return;
		}
		doAction(1569894142L /*WAIT_1*/, blendTime, onDoActionEnd -> scheduleState(state -> Wait_1(blendTime), 5000));
	}
	...
	protected void FailFindPath_1(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x4FBAE494L /*FailFindPath_1*/);
		doTeleport(EAIMoveDestType.Random, 0, 0, 1, 1);
		if (0 > 1000000) {
			if (changeState(state -> Progress_5(blendTime)))
				return;
	...
	protected void Move_Return_1(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.return_);
		setState(0xDFC8B5FEL /*Move_Return_1*/);
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> recoveryAndReturn(() -> {
			return false;
	...
	protected void Move_Random_1(double blendTime) {
		...
		if (getDistanceToSpawn() > 160) {
			if (changeState(state -> Move_Return_1(blendTime)))
				return;
		}
		doAction(847017389L /*WALK*/, blendTime, onDoActionEnd -> move(EAIMoveDestType.Random, 0, 0, 0, 50, 150, false, ENaviType.ground, () -> {
			setVariable(0x444DFF4EL /*_isFindPathCompleted*/, isFindPathCompleted());
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0 && getVariable(0xFDC61BCCL /*_FailFindPathCount*/) >= 3) {
				if (changeState(state -> Wait_1(0.3)))
					return true;
			}
			if (getVariable(0x444DFF4EL /*_isFindPathCompleted*/) == 0) {
				if (changeState(state -> FailFindPath_Logic_1(0.3)))
					return true;
			}
			return false;
