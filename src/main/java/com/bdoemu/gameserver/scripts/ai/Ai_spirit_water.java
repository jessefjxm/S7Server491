...
@IAIName("spirit_water")
public class Ai_spirit_water extends CreatureAI {
	public Ai_spirit_water(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void UnSummon(double blendTime) {
		...
		setVariable(0xFA9DA674L /*_isBattleMode*/, 0);
		clearAggro(true);
		doAction(1537067984L /*DIE_UNSUMMON*/, blendTime, onDoActionEnd -> scheduleState(state -> Die(blendTime), 1000));
	}
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (!checkOwnerEvadeEmergency() && getVariable(0xCBEEF8C7L /*_ownerDistance*/) > 1000) {
			if (changeState(state -> Chase_Owner(0.3)))
				return;
		}
		if (getVariable(0xFA9DA674L /*_isBattleMode*/) == 1) {
	...
	protected void Skill_Wait_Ing(double blendTime) {
		...
		doAction(696913725L /*SKILL_WAIT2*/, blendTime, onDoActionEnd -> scheduleState(state -> Skill_Wait_Ing(blendTime), 500));
	}
	...
	protected void Attack_LR(double blendTime) {
		...
		doAction(2820327238L /*BATTLE_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Attack_LR2(blendTime), 500));
	}
	...
	protected void Attack_CC(double blendTime) {
		...
		doAction(2820327238L /*BATTLE_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Attack_CC2(blendTime), 500));
	}
