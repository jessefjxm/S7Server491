...
@IAIName("summon_siegelargebuilding")
public class Ai_summon_siegelargebuilding extends CreatureAI {
	public Ai_summon_siegelargebuilding(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Working_Str(double blendTime) {
		...
		if (getVariable(0xE26EA7A0L /*_EndTime*/) >= 2400000) {
			if (changeState(state -> Working_End(blendTime)))
				return;
		}
		doAction(3058152320L /*WORKING_STR*/, blendTime, onDoActionEnd -> scheduleState(state -> Working_Str(blendTime), 24000));
	}
