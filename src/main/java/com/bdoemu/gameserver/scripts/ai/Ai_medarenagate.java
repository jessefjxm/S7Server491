...
@IAIName("medarenagate")
public class Ai_medarenagate extends CreatureAI {
	public Ai_medarenagate(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		doAction(2514775444L /*WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> AtaTime_Close_Wait(blendTime), 1000));
	}
	...
	protected void AtaTime_Open(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x589EE774L /*AtaTime_Open*/);
		doAction(1275795718L /*ONESTEP_OPEN*/, blendTime, onDoActionEnd -> {
			setVariable(0xDF434D85L /*_ISOpening*/, 1);
			scheduleState(state -> AtaTime_Open_Wait(blendTime), 1000);
		});
	}

	protected void AtaTime_Open_Wait(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xEFD7AAF5L /*AtaTime_Open_Wait*/);
		doAction(1214079745L /*WAIT_ONESTEP_OPEN*/, blendTime, onDoActionEnd -> {
			setVariable(0xDF434D85L /*_ISOpening*/, 1);
			scheduleState(state -> AtaTime_Open_Wait(blendTime), 1000);
		});
	}

	protected void AtaTime_Close(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xEBD1724BL /*AtaTime_Close*/);
		doAction(2755440460L /*ONESTEP_CLOSE*/, blendTime, onDoActionEnd -> {
			setVariable(0xDF434D85L /*_ISOpening*/, 0);
			scheduleState(state -> AtaTime_Close_Wait(blendTime), 1000);
		});
	}

	protected void AtaTime_Close_Wait(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xA7B029DAL /*AtaTime_Close_Wait*/);
		doAction(134297710L /*WAIT_ONESTEP_CLOSE*/, blendTime, onDoActionEnd -> {
			setVariable(0xDF434D85L /*_ISOpening*/, 0);
			scheduleState(state -> AtaTime_Close_Wait(blendTime), 1000);
		});
	}
	...
	@Override
	public EAiHandlerResult HandleOneStepOpen(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getVariable(0xDF434D85L /*_ISOpening*/) == 0) {
			if (changeState(state -> AtaTime_Open(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
	}
	...
	@Override
	public EAiHandlerResult HandleOneStepClose(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getVariable(0xDF434D85L /*_ISOpening*/) == 1) {
			if (changeState(state -> AtaTime_Close(0.1)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
	}
