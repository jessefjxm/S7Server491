...
@IAIName("npc_ship_merchant2")
public class Ai_npc_ship_merchant2 extends CreatureAI {
	public Ai_npc_ship_merchant2(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void FirstGo(double blendTime) {
		...
		doAction(2337397317L /*MOVE*/, blendTime, onDoActionEnd -> moveToWaypoint("carriage", "Olvia_Way_001 Olvia_Way_067", ENaviType.ground, () -> {
			return false;
	...
	protected void Go_Start(double blendTime) {
		...
		doAction(4060078600L /*MOVING_START*/, blendTime, onDoActionEnd -> scheduleState(state -> Go(blendTime), 100000));
	}
	...
	protected void End(double blendTime) {
		...
		doAction(155621870L /*MOVING_END*/, blendTime, onDoActionEnd -> scheduleState(state -> Go_Start(blendTime), 100000));
	}
