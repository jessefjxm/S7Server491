...
@IAIName("npc_client_bow")
public class Ai_npc_client_bow extends CreatureAI {
	public Ai_npc_client_bow(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void WaitHome(double blendTime) {
		...
		doAction(408565085L /*HOME*/, blendTime, onDoActionEnd -> scheduleState(state -> Vanish(blendTime), 100));
	}

	protected void Vanish(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xDF6B4F8AL /*Vanish*/);
		doAction(1926787974L /*HIDEMESH*/, blendTime, onDoActionEnd -> scheduleState(state -> Vanish(blendTime), 100));
	}
	...
	protected void MeshOffState(double blendTime) {
		...
		doAction(1926787974L /*HIDEMESH*/, blendTime, onDoActionEnd -> scheduleState(state -> MeshOffState(blendTime), 100));
	}
