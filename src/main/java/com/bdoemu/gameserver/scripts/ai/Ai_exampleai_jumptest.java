...
@IAIName("exampleai_jumptest")
public class Ai_exampleai_jumptest extends CreatureAI {
	public Ai_exampleai_jumptest(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Move_Chaser(double blendTime) {
		...
		if (target != null && target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) <= 1200 && target != null && isCreatureVisible(target, false)) {
			if(Rnd.getChance(30)) {
				if (changeState(state -> Battle_Attack1_Triple(0.3)))
					return;
			}
		}
	...
	protected void Battle_Attack1_Triple(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xEA93159AL /*Battle_Attack1_Triple*/);
		if (isTargetLost()) {
			if (changeState(state -> Lost_Target(blendTime)))
				return;
		}
		doAction(1556058307L /*BATTLE_ATTACK1_TRIPLE*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
