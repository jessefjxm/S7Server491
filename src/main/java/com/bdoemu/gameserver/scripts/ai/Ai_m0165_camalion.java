...
@IAIName("m0165_camalion")
public class Ai_m0165_camalion extends CreatureAI {
	public Ai_m0165_camalion(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Turn(double blendTime) {
		...
		doAction(2428216894L /*TURN_LEFT*/, blendTime, onDoActionEnd -> {
			if (target != null && getDistanceToTarget(target) < 400 && target != null && getTargetHp(target) > 0) {
				if(Rnd.getChance(15)) {
					if (changeState(state -> Battle_SpecialAttack(0.1)))
						return;
				}
			}
			if (target != null && getDistanceToTarget(target) < 300 && target != null && getTargetHp(target) > 0) {
				if(Rnd.getChance(15)) {
					if (changeState(state -> Jump_Back(0.1)))
						return;
				}
			}
			changeState(state -> Battle_Wait(blendTime));
	...
	protected void Jump_Left(double blendTime) {
		...
		doAction(3508935626L /*JUMP_LEFT*/, blendTime, onDoActionEnd -> {
			if (target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) < 200 && target != null && getAngleToTarget(target) >= -25 && target != null && getAngleToTarget(target) <= 25) {
				if(Rnd.getChance(20)) {
					if (changeState(state -> Jump_Right(0.1)))
						return;
				}
			}
			if (target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) < 200 && target != null && getAngleToTarget(target) >= -25 && target != null && getAngleToTarget(target) <= 25) {
				if(Rnd.getChance(20)) {
					if (changeState(state -> Jump_Left(0.1)))
						return;
				}
			}
			...
			changeState(state -> Battle_Attack4(blendTime));
		});
	}
	...
	protected void Jump_Right(double blendTime) {
		...
		doAction(801180298L /*JUMP_RIGHT*/, blendTime, onDoActionEnd -> {
			...
			changeState(state -> Battle_Attack1(blendTime));
		});
	}
	...
	protected void Jump_Back(double blendTime) {
		...
		doAction(2159179354L /*JUMP_BACK*/, blendTime, onDoActionEnd -> {
			if (target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) > 200 && target != null && getDistanceToTarget(target) < 600 && target != null && getAngleToTarget(target) >= -25 && target != null && getAngleToTarget(target) <= 25) {
				if (changeState(state -> Battle_ChargeAttack1(0.1)))
					return;
			}
			changeState(state -> Chaser_Run(blendTime));
		});
	}
	...
	protected void Attack_Logic(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) > 200 && target != null && getDistanceToTarget(target) <= 800) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getTargetHp(target) > 0 && target != null && getDistanceToTarget(target) > 700 && target != null && getDistanceToTarget(target) < 2500) {
			if (changeState(state -> Battle_PositionAttack1(0.1)))
				return;
		}
		if (target != null && getDistanceToTarget(target) < 200) {
			if (changeState(state -> Attack_Logic(0.1)))
				return;
		}
		if (target != null && getDistanceToTarget(target) > 200 && target != null && getDistanceToTarget(target) <= 800) {
			if (changeState(state -> Move_Chaser(0.1)))
	...
	protected void Battle_SpecialAttack(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x4171D0FBL /*Battle_SpecialAttack*/);
		if (isTargetLost()) {
			if (changeState(state -> TargetLost(blendTime)))
				return;
		}
		doAction(3673406979L /*BATTLE_SPECIALATTACK*/, blendTime, onDoActionEnd -> changeState(state -> Battle_Wait(blendTime)));
	}
	...
	protected void Battle_PositionAttack1_Ing(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) <= 500) {
			if (changeState(state -> Battle_PositionAttack1_End(0.1)))
				return;
		}
		doAction(3114108399L /*BATTLE_POSITIONATTACK1_ING*/, blendTime, onDoActionEnd -> chase(10, () -> {
			return false;
		}, onExit -> scheduleState(state -> Battle_PositionAttack1_Ing(blendTime), 100)));
	}
	...
	public EAiHandlerResult HandleTakeTeamDamage(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		if (getState() != 0x71F28994L /*Battle_Wait*/ && getState() != 0x39B3FBC2L /*Move_Chaser*/ && getState() != 0xB5FDC949L /*Battle_Attack1*/ && getState() != 0xD72BCC90L /*Battle_Attack2*/ && getState() != 0x94302B26L /*Battle_Attack3*/ && getState() != 0xF05721A5L /*Battle_Attack4*/ && getState() != 0x3A7FFDBBL /*Battle_ChargeAttack1*/ && getState() != 0x4171D0FBL /*Battle_SpecialAttack*/ && getState() != 0x3633FB8FL /*Battle_PositionAttack1*/ && getState() != 0xB0E9AF76L /*Battle_PositionAttack1_Ing*/ && getState() != 0x2953F032L /*Battle_PositionAttack1_End*/ && getState() != 0x1471881CL /*Move_ReturnToParent*/) {
			if (changeState(state -> Battle_Wait(0.3)))
				return EAiHandlerResult.CHANGE_STATE;
		}
		return EAiHandlerResult.BYPASS;
