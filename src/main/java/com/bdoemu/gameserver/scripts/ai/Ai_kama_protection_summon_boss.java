...
@IAIName("kama_protection_summon_boss")
public class Ai_kama_protection_summon_boss extends CreatureAI {
	public Ai_kama_protection_summon_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		doAction(927442757L /*WAIT1*/, blendTime, onDoActionEnd -> scheduleState(state -> Start_Action(blendTime), 1000));
	}
	...
	protected void Stage1_Wait(double blendTime) {
		...
		getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().Searching(getActor(), null));
		if (target == null) {
	...
	protected void Summon_Stage1(double blendTime) {
		...
		doAction(633711333L /*BATTLE_SUMMON_STAGE1*/, blendTime, onDoActionEnd -> {
			getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().Searching(getActor(), null));
