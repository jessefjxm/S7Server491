...
@IAIName("itemnewgoblin_boss")
public class Ai_itemnewgoblin_boss extends CreatureAI {
	public Ai_itemnewgoblin_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Detect_Target(double blendTime) {
		...
		doAction(2658402471L /*DETECT_ENEMY*/, blendTime, onDoActionEnd -> scheduleState(state -> Battle_State(blendTime), 1000));
	}
	...
	protected void Phase_1(double blendTime) {
		...
		if (changeState(state -> BigBangReady(blendTime)))
			return;
		changeState(state -> Battle_State(blendTime));
	}
	...
	protected void Phase_2(double blendTime) {
		...
		if (changeState(state -> CreateServant(blendTime)))
			return;
		changeState(state -> Battle_State(blendTime));
	}
	...
	protected void CreateServant(double blendTime) {
		...
		if (isTargetLost()) {
			if (changeState(state -> GoServantBomb(blendTime)))
				return;
		}
		doAction(1184791019L /*STANCE2_CHANGE*/, blendTime, onDoActionEnd -> changeState(state -> GoServantBomb(blendTime)));
	}

	protected void GoServantBomb(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0x828D3999L /*GoServantBomb*/);
		getObjects(EAIFindTargetType.Child, object -> true).forEach(consumer -> consumer.getAi().HandleGoMoveBombPos(getActor(), null));
		doAction(3739496196L /*STANCE2_CHANGE_SUMMON*/, blendTime, onDoActionEnd -> scheduleState(state -> Battle_State(blendTime), 1000));
	}

	protected void BigBangReady(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.idle);
		setState(0xDB87951CL /*BigBangReady*/);
		doAction(268748422L /*BIGBANG_READY*/, blendTime, onDoActionEnd -> scheduleState(state -> BigBang(blendTime), 3000));
	}

	protected void BigBang(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0x9D8AD3AL /*BigBang*/);
		if (isTargetLost()) {
			if (changeState(state -> Battle_State(blendTime)))
				return;
		}
		doAction(4029069781L /*BIGBANG*/, blendTime, onDoActionEnd -> changeState(state -> Battle_State(blendTime)));
	}
