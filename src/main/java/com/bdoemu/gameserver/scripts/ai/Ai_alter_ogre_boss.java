...
@IAIName("alter_ogre_boss")
public class Ai_alter_ogre_boss extends CreatureAI {
	public Ai_alter_ogre_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Rotate_Logic(double blendTime) {
		...
		if (checkParentInstanceTeamNo()) {
			if (findTarget(EAIFindTargetType.Monster, EAIFindType.normal, false, object -> getDistanceToTarget(object) < 4000 && getTargetCharacterKey(object) == 27134)) {
	...
	protected void Return_Shrine(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) <= 1000 && target != null && getDistanceToTarget(target) >= 400) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getSelfCombinePoint() != 13 && target != null && getDistanceToTarget(target) > 400) {
			if(Rnd.getChance(50)) {
		...
		if (target != null && getDistanceToTarget(target) <= 1000 && target != null && getDistanceToTarget(target) >= 400 && getVariable(0x5749908DL /*_JumpCount*/) >= 25) {
			if(Rnd.getChance(25)) {
				if (changeState(state -> Attack_Jump(0.3)))
					return;
			}
		}
		if (target != null && getDistanceToTarget(target) <= 1000 && target != null && getDistanceToTarget(target) >= 400 && getVariable(0x357581C9L /*_RangeCount*/) >= 15) {
			if(Rnd.getChance(25)) {
		...
		if (getSelfCombinePoint() != 13 && target != null && getDistanceToTarget(target) < 200) {
			if(Rnd.getChance(30)) {
	...
	protected void HighSpeed_Chaser(double blendTime) {
		...
		if (target != null && getDistanceToTarget(target) <= 900 && getVariable(0x3F487035L /*_HP*/) <= 50 && target != null && isCreatureVisible(target, false)) {
			if(Rnd.getChance(30)) {
	...
	@Override
	public EAiHandlerResult Attack_Mid(Creature sender, Integer[] eventData) {
		_sender = sender;
		Creature target = sender;
		clearAggro(true);
		if (findTarget(EAIFindTargetType.Monster, EAIFindType.nearest, false, object -> getDistanceToTarget(object) < 3500 && getTargetCharacterKey(object) == 27134)) {
