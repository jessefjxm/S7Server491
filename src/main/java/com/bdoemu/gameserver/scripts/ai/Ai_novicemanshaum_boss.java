...
@IAIName("novicemanshaum_boss")
public class Ai_novicemanshaum_boss extends CreatureAI {
	public Ai_novicemanshaum_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void InitialState(double blendTime) {
		...
		setVariable(0xD3505174L /*_Summon_Count*/, 5);
		...
		doAction(2235988577L /*START_WAIT*/, blendTime, onDoActionEnd -> scheduleState(state -> Summon_Vedir(blendTime), 100));
	}
	...
	protected void Summon_Vedir(double blendTime) {
		Creature target = getTarget();
		setBehavior(EAIBehavior.action);
		setState(0xC7CFF607L /*Summon_Vedir*/);
		doAction(2691846221L /*SUMMON_VEDIR*/, blendTime, onDoActionEnd -> changeState(state -> Start_Action(blendTime)));
	}
	...
	protected void Start_Action(double blendTime) {
		...
		getObjects(EAIFindTargetType.Child, object -> getDistanceToTarget(object) < 2000).forEach(consumer -> consumer.getAi().Appear(getActor(), null));
		doAction(3910697700L /*START_ACTION*/, blendTime, onDoActionEnd -> {
			getObjects(EAIFindTargetType.Child, object -> getDistanceToTarget(object) < 2000).forEach(consumer -> consumer.getAi().The_End(getActor(), null));
			scheduleState(state -> Start_Action_Logic(blendTime), 2000);
		});
	}
	...
	protected void Wait(double blendTime) {
		...
		getObjects(EAIFindTargetType.Child, object -> getDistanceToTarget(object) < 6000).forEach(consumer -> consumer.getAi().Return(getActor(), null));
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (getVariable(0xD94DEC74L /*_C4_Count*/) >= 20 && getPartyMembersCount()>= 1 && target != null && getTargetHeightdiff(target) > getVariable(0x27274876L /*_TargetBodyHeight*/) && target != null && getTargetHeightdiff(target) < getVariable(0x3715AB9DL /*_MyBodyHeight*/)) {
