...
@IAIName("novicestonegolem_boss")
public class Ai_novicestonegolem_boss extends CreatureAI {
	public Ai_novicestonegolem_boss(Creature actor, Map<Long, Integer> aiVariables) {
	...
	protected void Battle_Wait(double blendTime) {
		...
		if (target != null && getAngleToTarget(target) > 45 && target != null && getDistanceToTarget(target) < 500) {
			if(Rnd.getChance(20)) {
		...
		if (getVariable(0x3F487035L /*_Hp*/) <= 40 && target != null && getDistanceToTarget(target) > 100 && target != null && getDistanceToTarget(target) < 700) {
			if(Rnd.getChance(20)) {
				if (changeState(state -> Attack2(0.3)))
					return;
			}
		}
		if (getVariable(0x3F487035L /*_Hp*/) > 40 && target != null && getDistanceToTarget(target) > 200 && target != null && getDistanceToTarget(target) < 700) {
			if(Rnd.getChance(20)) {
		...
		if (target != null && getDistanceToTarget(target) < 1500 && target != null && getDistanceToTarget(target) > 500) {
			if(Rnd.getChance(30)) {
	...
	protected void Attack(double blendTime) {
		...
		doAction(31162842L /*BATTLE_ATTACK1*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack1(double blendTime) {
		...
		doAction(2323327157L /*BATTLE_ATTACK2*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack2(double blendTime) {
		...
		doAction(931985982L /*BATTLE_ATTACK3*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Jump(double blendTime) {
		...
		doAction(2557095778L /*BATTLE_ATTACK_JUMP*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Kick(double blendTime) {
		...
		doAction(3572319367L /*BATTLE_ATTACK_KICK*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Roll(double blendTime) {
		...
		doAction(3738328107L /*BATTLE_ATTACK_ROLL*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Turn_180(double blendTime) {
		...
		doAction(3442683276L /*BATTLE_ATTACK_TURN_180*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Turn_Left(double blendTime) {
		...
		doAction(3329719241L /*BATTLE_ATTACK_TURN_LEFT*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Turn_Right(double blendTime) {
		...
		doAction(460887176L /*BATTLE_ATTACK_TURN_RIGHT*/, blendTime, onDoActionEnd -> changeState(state -> Aggro_Change_Logic(blendTime)));
	}
	...
	protected void Attack_Chop(double blendTime) {
		...
		doAction(1036356655L /*ATTACK_CHOP*/, blendTime, onDoActionEnd -> {
			if (changeState(state -> Aggro_Change_Logic(0.5)))
				return;
			changeState(state -> Aggro_Change_Logic(blendTime));
		});
	}
