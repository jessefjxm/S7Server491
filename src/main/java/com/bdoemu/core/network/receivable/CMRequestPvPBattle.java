...
public class CMRequestPvPBattle extends ReceivablePacket<GameClient> {
    private int gameObjectId;

    public CMRequestPvPBattle(final short opcode) {
        super(opcode);
    }
        ...
        if (player != null) {
            ...
            if (opponent != null) {
                ...
                if (player.hasPvpMatch()) {
                    player.sendPacket(new SMNak(EStringTable.eErrNoPvPMatchCantRequestDuringPvPMatch, CMAcceptPvPBattle.class));
                    return;
                }
                ...
                PvPBattleService.getInstance().askDuel(player, opponent);
            }
        }
    }
}
