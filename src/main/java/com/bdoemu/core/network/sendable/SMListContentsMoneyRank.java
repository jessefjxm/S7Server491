...
public class SMListContentsMoneyRank extends SendablePacket<GameClient> {
    private List<Player> players;

    public SMListContentsMoneyRank(final List<Player> players) {
    ...
    protected void writeBody(final SendByteBuffer buffer) {
        ...
        for (int i = 0; i < 30; ++i) {
            ...
            this.writeMoneyRankData(buffer, player);
        }
    }

    private void writeMoneyRankData(final SendByteBuffer buffer, final Player player) {
        ...
        buffer.writeQ((player == null) ? 0L : player.getPlayerBag().getAllMoney());
